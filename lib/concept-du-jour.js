...
// concept_name déjà publiés dans les RECENT_REPEAT_AVOIDANCE_DAYS derniers
// jours — filet de sécurité même si l'IA ignore la consigne du prompt.
function validateSingleConcept(raw, validTopicIds, knownUrls, recentConceptsNormalized) {
  ...
  if (recentConceptsNormalized && recentConceptsNormalized.has(normalizeIdentity(raw.concept_name))) {
    return { ok: false, reason: `concept_name déjà utilisé dans les ${RECENT_REPEAT_AVOIDANCE_DAYS} derniers jours.` };
  }
...
function validateConceptDuJourResponse(raw, topics, recentConceptsNormalized) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
  ...
  for (const item of raw.concepts) {
    const result = validateSingleConcept(item, validTopicIds, knownUrls, recentConceptsNormalized);
...
function createConceptDuJourService(deps) {
  ...
  async function runGeneration(dateKey) {
    ...
    // et vérifiés à nouveau après coup (validateSingleConcept), pour ne
  ...
  async function attemptGenerationOnce(usableTopics, recentConcepts, recentConceptsNormalized) {
    let raw;
    try {
      const prompt = buildConceptDuJourPrompt(usableTopics, recentConcepts);
    ...
    const validation = validateConceptDuJourResponse(raw, usableTopics, recentConceptsNormalized);
