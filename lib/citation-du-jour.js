...
// IA, validation stricte, stockage Supabase, anti-concurrence. Jumeau de
// lib/concept-du-jour.js (même architecture, mêmes garde-fous), adapté à
// une citation authentique d'un auteur célèbre plutôt qu'un concept — le
// champ le plus sensible ici est quote_text/quote_author : une citation
// fausse ou mal attribuée est une désinformation directe visant une
// personne réelle, cf. le garde-fou "insufficient par défaut" dans
// prompts/citation-du-jour.js. Ne dépend jamais de server.js — toutes ses
// dépendances (client Supabase, appel OpenAI, logger, horloge, récupération
// des sujets du jour) lui sont injectées via createCitationDuJourService(deps).

const { buildCitationDuJourPrompt } = require("../prompts/citation-du-jour");

const TABLE = "citation_du_jour";
const MAX_TOPICS_SENT = 10;
// gpt-4.1-mini, cohérent avec les quatre autres rubriques Éclairages
...
// risque (moins de citations produites = moins de risque de mauvaise
// attribution cumulé) et le coût.
...
const CITATION_ITEM_ALLOWED_KEYS = new Set([
  ...
  "quote_text", "quote_author", "quote_origin",
  "quote_explanation", "shared_mechanism", "essential_difference",
  "conclusion", "sources"
]);
...
// d'actualité lui-même en parle déjà — même garde-fou que les quatre autres
// rubriques.
...
// --- Image (portrait de l'auteur de la citation) — même mécanisme que dans
// lib/concept-du-jour.js : API publique Wikipedia, aucune clé requise,
// jamais générée ni devinée par l'IA. Dupliqué plutôt que partagé (cf. les
// quatre autres rubriques, même convention de "fichiers jumeaux" que le
// reste de cette fonctionnalité).
...
async function queryWikipediaImage(lang, title) {
  ...
  // Même filtre que les quatre autres rubriques : une image "de tête" en
  // SVG (carte, drapeau, blason) n'est jamais une vraie photo/portrait.
...
// Valide UNE citation (champs présents, longueurs, HTML, comparaison
// sensible, sources). knownUrls et validTopicIds sont calculés une fois
// pour tout le tableau par l'appelant.
function validateSingleCitation(raw, validTopicIds, knownUrls) {
  ...
  const coreWordCount = countWords(raw.quote_explanation) + countWords(raw.shared_mechanism)
  ...
  const citationText = stripDiacritics(`${raw.quote_text} ${raw.quote_explanation} ${raw.shared_mechanism} ${raw.essential_difference} ${raw.conclusion}`).toLowerCase();
  const introducesSensitiveTermItself = SENSITIVE_TERMS.some((term) => currentTopicText.includes(term));
  const usesSensitiveTermAsAnalogy = SENSITIVE_TERMS.some((term) => citationText.includes(term));
  ...
  return {
    ...
    data: {
      ...
      quote_explanation: raw.quote_explanation.trim(),
...
// autorisées, 1 à MAX_CITATIONS_PER_DAY citation(s) (jamais 0, jamais un
...
function validateCitationDuJourResponse(raw, topics) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
  ...
  if (!Array.isArray(raw.citations) || raw.citations.length < 1 || raw.citations.length > MAX_CITATIONS_PER_DAY) {
  ...
  for (const item of raw.citations) {
    const result = validateSingleCitation(item, validTopicIds, knownUrls);
    if (!result.ok) return result;
    if (usedTopicIds.has(result.data.current_topic_id)) {
      return { ok: false, reason: `plusieurs citations portent sur le même sujet (${result.data.current_topic_id}).` };
    }
...
function createCitationDuJourService(deps) {
  ...
  function toClientResult(row) {
    ...
    switch (row.status) {
      ...
      case "insufficient":
        return {
          status: "insufficient",
          reason: (row.content && row.content.reason) || row.error_message || "Aucune citation pertinente n'a pu être établie aujourd'hui."
        };
      case "generating":
    ...
    switch (row.status) {
      ...
      case "insufficient":
        return {
          status: "insufficient",
          date: row.date,
          reason: (row.content && row.content.reason) || row.error_message || "Aucune citation pertinente n'a pu être établie ce jour-là."
        };
      case "generating":
  ...
  async function markPublished(dateKey, data) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        // jointe des sujets couverts par la/les citation(s) publiée(s) — la
        // vraie source de vérité reste content.citations[].current_topic_id.
  ...
  // portrait de l'auteur cité (Wikipedia), et si rien de pertinent n'est
  // trouvé, repli sur l'image déjà publiée avec l'arène du sujet d'actualité
  // (illustration de l'actu, pas de l'auteur — mais réelle et déjà vérifiée
  // par le site). Ne fait jamais échouer la publication : sans aucune des
  // deux, publié sans image.
  async function attachQuoteImageToOne(citation, matchingTopic) {
    try {
    ...
    if (knownPressImageUrl) {
      return {
        ...citation,
        quote_author_image_url: knownPressImageUrl,
        quote_author_image_page_url: sourceUrl || null,
        quote_author_image_source: "press",
        quote_author_image_credit: (primarySource && primarySource.name) || domainFromUrl(sourceUrl) || "source de l'actualité"
      };
    }
    ...
    if (sourceUrl) {
      try {
        ...
        if (preview && preview.imageUrl) {
          return {
            ...citation,
            quote_author_image_url: preview.imageUrl,
            quote_author_image_page_url: sourceUrl,
            quote_author_image_source: "press",
            quote_author_image_credit: preview.siteName || (primarySource && primarySource.name) || domainFromUrl(sourceUrl) || "source de l'actualité"
          };
        }
      } catch (err) {
        logger.error("[citation-du-jour] aperçu image de presse :", err.message);
      }
    }
  ...
  // Enrichit chaque citation du tableau — en série plutôt qu'en parallèle JS
  // pour rester sous le radar des limites de débit de l'API Wikipedia
  // publique malgré le petit nombre d'appels.
  async function attachQuoteImages(data, usableTopics) {
    const topicsById = new Map((usableTopics || []).map((t) => [String(t.id), t]));
    const enrichedCitations = [];
    for (const citation of data.citations) {
      const matchingTopic = topicsById.get(citation.current_topic_id);
      enrichedCitations.push(await attachQuoteImageToOne(citation, matchingTopic));
    }
  ...
  async function runGeneration(dateKey) {
    ...
    // Les quatre autres rubriques Éclairages ont toujours priorité sur le
    // choix du sujet : la citation du jour attend leur résultat du jour
    ...
    // sujet à filtrer : inutile d'attendre les quatre autres rubriques si
    // de toute façon rien n'est exploitable aujourd'hui.
    ...
    if (!usableTopics.length) {
      const reason = hadUsableTopicsBeforeExclusion && excludedTopicIds.size
        ? "Tous les sujets exploitables du jour sont déjà couverts par le parallèle historique, la pensée philosophique, le mécanisme sociologique ou le concept du jour."
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      ...
      lastOutcome = outcome;
      if (outcome.ok) {
        const enrichedData = await attachQuoteImages(outcome.data, usableTopics);
  ...
  async function attemptGenerationOnce(usableTopics) {
    let raw;
    try {
      const prompt = buildCitationDuJourPrompt(usableTopics);
    ...
    const validation = validateCitationDuJourResponse(raw, usableTopics);
