...
// l'appelant.
function validateSingleCitation(raw, validTopicIds) {
...
function validateCitationDuJourResponse(raw, topics) {
  ...
  for (const item of raw.citations) {
    const result = validateSingleCitation(item, validTopicIds);
...
function createCitationDuJourService(deps) {
  ...
  async function generateIfNeeded(date, options = {}) {
    ...
    if (!claim.claimed) return toClientResult(await repairPublishedQuoteImages(claim.row));
