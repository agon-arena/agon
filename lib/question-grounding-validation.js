...
//
// options.skipAnswerAlignmentCheck (Phase 2.1, correctif du 04/09/2026,
// diagnostic réel "Grande Muraille de Chine" — generationId cc53c0ed025b2b54) :
// optionnel, false par défaut (comportement STRICTEMENT inchangé pour tout
// appelant qui ne le fournit pas, notamment le pipeline legacy). Quand true,
// saute UNIQUEMENT la boucle "réponse vs affirmation citée" ci-dessous
// (answer_not_in_claim / numeric_claim_not_supported / excessive_precision)
// — jamais les contrôles "affirmation vs source" au-dessus (missing_
// supporting_claim / unknown_source / too_many_sources / claim_not_grounded_
// in_source), qui restent TOUJOURS actifs et servent de garde-fou défensif
// que le mécanisme d'override a bien fonctionné.
//
// Diagnostic réel (deux exemples concrets, 04/09/2026) : quand
// supporting_claim est FORCÉ à evidence_text (V1 evidence grounding,
// applyEvidenceGroundingOverride), ce texte est un COURT extrait source
// (jusqu'à 500 caractères, souvent un fragment de phrase — ex. "et d'abris
// pour se protéger", littéralement tronqué en tête de citation), choisi et
// écrit par l'appel curriculum_generation pour prouver knowledgeTarget —
// jamais rédigé en pensant à la formulation exacte que la réponse d'une
// question, générée séparément et plus tard, emploiera. Une réponse
// factuellement exacte et fidèle à knowledgeTarget (déjà vérifié grounded
// contre la source par validateKnowledgeEvidence, EN AMONT et de façon
// déterministe) peut ainsi légitimement reformuler ("à protéger les
// personnes qui s'y trouvaient" pour "pour se protéger") ou accorder
// différemment ("construite" singulier pour "construites" pluriel dans la
// source) sans qu'aucune information fausse ou non soutenue ne soit
// introduite — cette boucle rejetait alors à tort une réponse correcte,
// jamais un vrai défaut de traçabilité (déjà garanti par un mécanisme plus
// fort et indépendant en amont). Ne s'applique JAMAIS quand supporting_claim
// n'est pas un evidence_text forcé : le pipeline V3 "libre" (le modèle
// compose lui-même son supporting_claim, en le sachant vérifié) n'est
// jamais concerné par ce correctif, jamais assoupli.
function validateQuestionGrounding(question, sourcesById, options = {}) {
  ...
  if (options.skipAnswerAlignmentCheck) return { ok: true, evidence: claim.slice(0, 240) };
...
// ── Grounding PÉDAGOGIQUE (V2 pipeline progressif, audit du 03/09/2026,
// section 6/7 de la demande) : distinct de validateQuestionGrounding
// ci-dessus (qui vérifie qu'une question est tracée à la SOURCE brute) et de
// validateKnowledgeEvidence (qui vérifie qu'une CONNAISSANCE du curriculum
// est tracée à la source, en amont de toute rédaction) — ici on vérifie
// qu'une QUESTION DÉJÀ GÉNÉRÉE porte bien sur une connaissance réellement
// enseignée dans le PARAGRAPHE tel qu'affiché à l'utilisateur (jamais la
// source brute, jamais la mémoire du modèle). Principe explicite de la
// demande : "Mnoria interroge uniquement sur ce qu'il vient d'enseigner."
//
// Volontairement déterministe et mécanique (demande explicite : "éviter une
// usine à gaz de similarité sémantique") — containment lexical du
// knowledgeTarget dans le texte du paragraphe, MÊME famille de mesure que
// tokenContainment (lib/qcm-quality.js), jamais un jugement IA ni un second
// appel modèle. Un knowledgeTarget est une phrase factuelle courte, en
// principe recopiée sans reformulation dans le champ `knowledgeTarget` de la
// question (cf. buildQuestionsFromKnowledgePrompt, "reprend EXACTEMENT le
// texte du fait numéroté") : si le paragraphe a réellement enseigné cette
// connaissance, une large majorité de ses mots significatifs doivent
// apparaître littéralement dans le texte du paragraphe.
const MIN_PARAGRAPH_GROUNDING_OVERLAP = 0.5;
// En dessous de ce nombre de tokens significatifs, la mesure de containment
// n'a aucune valeur diagnostique (ex. un knowledgeTarget très court après
// filtrage des mots vides) — jamais rejeté sur un signal aussi faible.
const MIN_PARAGRAPH_GROUNDING_TOKENS = 3;

// Dupliqué de lib/qcm-quality.js tokenSet/tokenContainment (jamais importé
// depuis là : ce fichier est déjà consommé PAR lib/qcm-quality.js, cf.
// commentaire de tête — un cycle inverse créerait la même dépendance
// circulaire déjà évitée pour normalizeComparisonText ci-dessus).
function paragraphGroundingTokenSet(value) {
  return new Set(normalizeComparisonText(value).split(/[^a-z0-9]+/).filter((token) => token.length > 2));
}

function paragraphGroundingContainment(shortSet, longSet) {
...
// `question` : une question déjà générée, portant `knowledgeTarget`.
// `paragraphText` : texte brut du (des) bloc(s) de paragraphe pertinent(s)
// pour le NIVEAU de cette question uniquement (jamais le texte cumulatif des
// niveaux précédents — cf. rapport, section "Approfondi/Expert", grounding
// local à chaque bloc). Absent/vide : no-op permissif (`ok:true`) — ce
// contrôle ne s'applique qu'aux blocs progressifs qui fournissent
// réellement un paragraphe, jamais au pipeline legacy qui ne l'appelle pas.
function validateParagraphGrounding(question, paragraphText) {
  const paragraph = String(paragraphText || "").trim();
  if (!paragraph) return { ok: true };
  const target = String(question?.knowledgeTarget || "").trim();
  if (!target) return { ok: true };
  const targetTokens = paragraphGroundingTokenSet(target);
  if (targetTokens.size < MIN_PARAGRAPH_GROUNDING_TOKENS) return { ok: true };
  const paragraphTokens = paragraphGroundingTokenSet(paragraph);
  const overlap = paragraphGroundingContainment(targetTokens, paragraphTokens);
  if (overlap < MIN_PARAGRAPH_GROUNDING_OVERLAP) {
    return {
      ok: false,
      reason: "not_taught_in_paragraph",
      detail: `knowledgeTarget insuffisamment représenté dans le paragraphe (recouvrement ${Math.round(overlap * 100)}%).`
    };
  }
  return { ok: true };
}
...
module.exports = {
  ...
  MIN_PARAGRAPH_GROUNDING_OVERLAP,
