...
const DEFAULT_CONFIG = Object.freeze({
  pipelineVersion: "coeus-items-v2-captions",
...
function normalizeSubtitleCues(rawCues) {
  if (!Array.isArray(rawCues) || rawCues.length < 1 || rawCues.length > 10) return null;
  const cues = [];
  let previousEnd = 0;
  for (const raw of rawCues) {
    ...
    const kind = raw?.kind === "question" || raw?.kind === "answer" ? raw.kind : null;
    if (!Number.isFinite(start) || !Number.isFinite(end) || start < 0 || end <= start || start < previousEnd || !text || text.length > 500 || !kind) {
      return null;
    }
    cues.push({ start, end, text, kind });
    previousEnd = end;
  }
  return cues;
}
...
async function finalizeNoesVideo({ supabase, clients, config }, row, jobOutput) {
  ...
  await noesRepository.markNoesVideoReady(supabase, row.id, {
    ...
    subtitleCues: normalizeSubtitleCues(jobOutput.subtitle_cues)
  });
