...
// sujets publiés aujourd'hui sur Agôn, mais présentation volontairement
...
const OEUVRE_ITEM_ALLOWED_KEYS = new Set([
  ...
  "artwork_description", "artist_presentation", "sources"
]);
...
// --- Image (l'œuvre elle-même, en priorité — jamais générée ni devinée
...
// ici l'œuvre elle-même qui est l'image centrale — l'artiste n'est tenté
// qu'en repli si aucune image dédiée à l'œuvre n'est trouvée.
...
// Cherche d'abord une reproduction de l'œuvre elle-même (artwork_title, en
// vérifiant que le nom de l'artiste apparaît bien dans le résumé de la page
// trouvée — cf. queryWikipediaImage), puis à défaut un portrait de
// l'artiste (artist_name, sans cette vérification supplémentaire : une
// recherche par nom d'artiste retombe par nature sur une page consacrée à
// cet artiste). Best-effort : ne bloque jamais la publication, se contente
// de publier sans image en cas d'échec/absence de résultat pertinent.
async function defaultFetchArtworkImage(artworkTitle, artistName) {
  ...
  if (cleanTitle) {
    for (const lang of ["fr", "en"]) {
      try {
        ...
        if (result) return { ...result, tier: "artwork" };
      } catch (e) {
  ...
  if (cleanArtist) {
    for (const lang of ["fr", "en"]) {
      try {
        const result = await queryWikipediaImage(lang, cleanArtist);
        if (result) return { ...result, tier: "artist" };
      } catch (e) {
...
// validTopicIds est calculé une fois pour tout le tableau par l'appelant.
function validateSingleOeuvre(raw, validTopicIds) {
...
function validateOeuvreArtDuJourResponse(raw, topics) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
  ...
  for (const item of raw.oeuvres) {
    const result = validateSingleOeuvre(item, validTopicIds);
...
function createOeuvreArtDuJourService(deps) {
    ...
    try {
      ...
      if (image && image.imageUrl) {
        return {
          ...
          artwork_image_source: "wikipedia",
          artwork_image_credit: "Wikipedia"
        };
      }
    } catch (err) {
      logger.error("[oeuvre-art-du-jour] recherche image Wikipedia :", err.message);
    }
  ...
  async function runGeneration(dateKey) {
    ...
    if (!usableTopics.length) {
      const reason = hadUsableTopicsBeforeExclusion && excludedTopicIds.size
        ? "Tous les sujets exploitables du jour sont déjà couverts par une autre rubrique Éclairages."
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      ...
      lastOutcome = outcome;
      if (outcome.ok) {
        const enrichedData = await attachArtworkImages(outcome.data);
  ...
  async function attemptGenerationOnce(usableTopics) {
    let raw;
    try {
      const prompt = buildOeuvreArtDuJourPrompt(usableTopics);
    ...
    const validation = validateOeuvreArtDuJourResponse(raw, usableTopics);
