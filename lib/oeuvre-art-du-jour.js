...
// fiable de l'œuvre n'est trouvée (ni pour elle ni pour aucune des œuvres
// essayées sur les tentatives suivantes, cf. runGeneration plus bas), la
// rubrique reste "insufficient" pour la journée plutôt que d'être publiée
// sans image ou avec un portrait/une illustration voisine (revu le
// 09/09/2026, "si pas d'image fiable, on ne met pas d'œuvre d'art du jour").
...
function createOeuvreArtDuJourService(deps) {
  ...
  async function runGeneration(dateKey) {
    ...
    // autre plutôt que de retomber dessus.
    // Revu le 09/09/2026 ("si pas d'image, ou pas d'image fiable, on ne met
    // pas d'œuvre d'art du jour") : l'ancien filet de secours qui publiait
    // quand même sans image après épuisement des tentatives est retiré —
    // l'image EST le cœur de cette rubrique (cf. commentaire de tête de
    // fichier), un texte seul sans reproduction fiable de l'œuvre ne vaut
    // plus la peine d'être publié. lastValidWithoutImage n'est donc plus
    // republié en dernier recours : seul son motif ("aucune image fiable
    // trouvée après N tentatives") sert désormais de raison à "insufficient".
    let lastOutcome = null;
    let lastValidWithoutImageReason = null;
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      ...
      if (outcome.ok) {
        ...
        lastValidWithoutImageReason = `Aucune reproduction fiable trouvée pour « ${enrichedData.oeuvres.map((o) => o.artwork_title).join(", ")} ».`;
    ...
    if (lastValidWithoutImageReason) {
      await markInsufficient(dateKey, lastValidWithoutImageReason);
      return { status: "insufficient", reason: lastValidWithoutImageReason };
    }
