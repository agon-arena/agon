...
const PHILOSOPHICAL_IMAGE_REPAIR_COOLDOWN_MS = 15 * 60 * 1000;
...
// philosophical_concept déjà publiés dans les RECENT_REPEAT_AVOIDANCE_DAYS
// derniers jours — filet de sécurité même si l'IA ignore la consigne du
// prompt.
function validateSinglePensee(raw, validTopicIds, knownUrls, recentConceptsNormalized) {
  ...
  if (recentConceptsNormalized && recentConceptsNormalized.has(normalizeIdentity(raw.philosophical_concept))) {
    return { ok: false, reason: `philosophical_concept déjà utilisé dans les ${RECENT_REPEAT_AVOIDANCE_DAYS} derniers jours.` };
  }
...
function validatePenseePhilosophiqueResponse(raw, topics, recentConceptsNormalized) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
  ...
  for (const item of raw.pensees) {
    const result = validateSinglePensee(item, validTopicIds, knownUrls, recentConceptsNormalized);
...
function createPenseePhilosophiqueService(deps) {
    ...
    try {
      ...
      if (image && image.imageUrl) {
        return {
          ...
          philosophical_concept_image_checked_at: checkedAt
        };
      }
    } catch (err) {
  ...
  function penseeNeedsImageRepair(pensee) {
    if (!pensee || pensee.philosophical_concept_image_url || (!String(pensee.philosopher_name || "").trim() && !String(pensee.philosophical_concept || "").trim())) return false;
    const checkedAt = new Date(pensee.philosophical_concept_image_checked_at || 0).getTime();
    return !Number.isFinite(checkedAt) || Date.now() - checkedAt >= PHILOSOPHICAL_IMAGE_REPAIR_COOLDOWN_MS;
  }

  async function repairPublishedConceptImages(row) {
    if (!row || row.status !== "published" || !Array.isArray(row.content && row.content.pensees)) return row;
    if (!row.content.pensees.some(penseeNeedsImageRepair)) return row;
    ...
    const repairPromise = (async () => {
      ...
      try {
        ...
        logger.error("[pensee-philosophique] récupération sujets pour réparation image :", err.message);
        return null;
      }
      const topicsById = new Map((Array.isArray(topics) ? topics : []).map((topic) => [String(topic.id), topic]));
      const pensees = [];
      for (const pensee of row.content.pensees) {
        pensees.push(penseeNeedsImageRepair(pensee)
          ? await attachConceptImageToOne(pensee, topicsById.get(String(pensee.current_topic_id)))
          : pensee);
      }
      const content = { ...row.content, pensees };
      const { error } = await supabase.from(TABLE).update({ content, updated_at: new Date().toISOString() }).eq("date", dateKey).eq("status", "published");
      if (error) {
        logger.error("[pensee-philosophique] écriture réparation image :", error.message);
        return null;
      }
      return content;
    })();
  ...
  async function runGeneration(dateKey) {
    ...
    // et vérifiés à nouveau après coup (validateSinglePensee), pour ne
    ...
    const { raw: recentConcepts, normalized: recentConceptsNormalized } = await fetchRecentEclairagesIdentities({
      ...
      table: TABLE,
      contentKey: "pensees",
      identityField: "philosophical_concept",
  ...
  async function attemptGenerationOnce(usableTopics, recentConcepts, recentConceptsNormalized) {
    let raw;
    try {
      const prompt = buildPenseePhilosophiquePrompt(usableTopics, recentConcepts);
    ...
    const validation = validatePenseePhilosophiqueResponse(raw, usableTopics, recentConceptsNormalized);
