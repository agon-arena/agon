...
// Plusieurs pensées possibles par jour, mais jamais un nombre fixe : le
// prompt instruit explicitement de n'en publier qu'autant que de sujets
// s'y prêtant vraiment (1 à 3), jamais pour atteindre un quota.
const MAX_PENSEES_PER_DAY = 3;
...
function validatePenseePhilosophiqueResponse(raw, topics) {
  ...
  if (!Array.isArray(raw.pensees) || raw.pensees.length < 1 || raw.pensees.length > MAX_PENSEES_PER_DAY) {
    return { ok: false, reason: `pensees doit être un tableau de 1 à ${MAX_PENSEES_PER_DAY} élément(s).` };
  }
  ...
  async function runGeneration(dateKey) {
    ...
    if (!usableTopics.length) {
      const reason = hadUsableTopicsBeforeExclusion && excludedTopicIds.size
        ? "Tous les sujets exploitables du jour sont déjà couverts par le parallèle historique."
