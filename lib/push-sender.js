...
async function processPendingPushEvents(supabase, vapidConfig, { limit = 3 } = {}) {
  ...
  for (const event of events) {
    ...
    if (!subscriptionRow) {
      ...
      results.push({ id: event.id, status: "failed", reason: "no-active-subscription" });
      continue;
    }
    ...
    try {
      ...
      results.push({ id: event.id, status: "sent", subscription_id: subscriptionRow.id });
    } catch (error) {
      const payload = {
        ...(event.payload || {}),
      ...
      await updateNotificationEventStatus(supabase, event.id, "failed", payload);
      results.push({ id: event.id, status: "failed", reason: payload.push_error });
    }
  }
