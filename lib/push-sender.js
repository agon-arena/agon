...
function buildEventPayload(event) {
  ...
  const body = payload.message || "Nouvelle activité sur mnoria.";
  const notificationId = payload.notification_id || payload.notificationId || null;

  let url = "/notifications";
  if (event.event_type === "learning_digest") {
    url = "/qcm-du-jour";
  } else if (event.event_type === "notion_quiz_ready") {
    // Scroll + clignotement sur la connaissance tout juste ajoutée (photo/PDF/
    // texte/URL/YouTube/manuel ou génération IA), une fois la page apprentissage
    // rechargée — demande du 02/09/2026, même comportement que le clic sur un
    // sujet du catalogue (cf. qcm-du-jour.html highlightLatestQuizOnLoad).
    url = "/qcm-du-jour?highlightLatest=1";
  } else if (event.event_type === "notion_quiz_failed") {
    url = "/qcm-du-jour";
  } else if (event.comment_id && event.debate_id) {
  ...
  // Filet de sécurité : le service worker tente déjà de marquer la notification
  // lue en tâche de fond au clic, mais ce fetch peut être interrompu par l'OS
  // (notamment iOS, qui ne garantit pas l'exécution du service worker après un
  // clic depuis une app totalement fermée). En passant aussi l'id en query
  // param, la page elle-même confirme la lecture une fois ouverte au premier plan.
  if (notificationId) {
    url += (url.includes("?") ? "&" : "?") + `notif=${notificationId}`;
  }
