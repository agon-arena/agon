...
function buildP5(insights) {
  ...
  const observationSchema = isOpen
    ? `"campAObservation": "<1-2 phrases sur les idées partagées uniquement, rédigées comme la suite naturelle du constat principal qui précède : n'emploie jamais le mot camp, n'ajoute pas de guillemets autour d'Idées partagées, ne commence JAMAIS par 'Les idées partagées' (déjà annoncé par le titre affiché juste au-dessus), et désigne-les par 'les contributions' plutôt que par 'les idées' (pour éviter la tautologie 'des idées qui présentent/proposent des idées') — attribue-leur directement une qualité (popularité, robustesse, score), sans verbe de présentation type 'présentent/proposent des idées', avec une majuscule initiale, par exemple : Ces contributions affichent une robustesse variable malgré leur popularité... / Malgré une popularité marquée, ces contributions restent inégales sur le plan argumentatif...>",
  ...
  const positionRule = isOpen
    ? '- Dans campAObservation : ne jamais écrire "camp", "camp A", "camp Idées partagées" ou "camp \\"Idées partagées\\"", et ne jamais commencer la phrase par "Les idées partagées" (déjà énoncé par le titre affiché juste au-dessus de ce texte). Désigne les idées par "les contributions" plutôt que par "les idées", pour éviter toute tautologie du type "des idées qui présentent/proposent des idées". Attribue-leur directement une qualité (popularité, robustesse, score) sans verbe de présentation, avec une majuscule en début de phrase.'
