...
function parseYoutubeVideoUrl(rawUrl) {
  ...
  try { parsed = new URL(trimmed); }
    ...
    // Lien collé sans "https://" (ex. "youtu.be/xxxx" ou "m.youtube.com/watch?v=xxxx",
    // très fréquent selon d'où le lien est copié) : new URL() le rejette tel quel alors
    // qu'il est par ailleurs valide — un seul essai de repli avant d'abandonner
    // (demande du 02/09/2026, "URL YouTube invalide" sur un lien pourtant correct).
    try { parsed = new URL(`https://${trimmed}`); }
    catch (retryError) { throw new YoutubeKnowledgeError("invalid_url", "URL YouTube invalide."); }
  }
...
function normalizeTranscript(segments) {
  ...
  for (const segment of (Array.isArray(segments) ? segments : [])) {
    ...
    previous = text;
    // youtube-transcript 1.3+ renvoie offset/duration en MILLISECONDES (format
    // srv3 "<p t= d=>", prioritaire depuis ce changement de version) — jamais
    // plus en secondes comme l'ancien format XML classique auquel la lib ne
    // se replie désormais que si le format récent n'a rien trouvé. Sans cette
    // conversion, une vidéo de 52 minutes calculait ~52 000 minutes et
    // dépassait à tort la limite de 2h (demande du 02/09/2026).
    const offsetSeconds = Math.max(0, Number(segment?.offset) || 0) / 1000;
    const durationSecondsForSegment = Math.max(0, Number(segment?.duration) || 0) / 1000;
    cleaned.push({ text, offset: offsetSeconds, duration: durationSecondsForSegment });
  }
