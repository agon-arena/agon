...
// Bug corrigé le 31/08/2026 (trouvaille en testant lib/web-search-grounding.js
// en conditions réelles) : depuis que Node active Happy Eyeballs
// (autoSelectFamily) par défaut, https.request appelle ce lookup avec
// {all:true} et attend un TABLEAU d'adresses en retour — l'ancien
// callback(null, address, family) (format legacy, un seul candidat)
// faisait échouer silencieusement toute requête HTTPS pinnée avec "Invalid
// IP address: undefined" (masqué en "fetch_failed" par le handler d'erreur
// plus bas), y compris vers des hôtes parfaitement valides comme Wikipedia.
// Jamais détecté par les tests unitaires existants (requestPage y est
// mocké, cf. test/url-knowledge.test.js) — seul un vrai appel réseau le
// révèle. Extraite en fonction nommée pour être testable isolément, sans
// vrai réseau (cf. test/url-knowledge.test.js).
function buildPinnedLookup(address) {
  return (hostname, lookupOptions, callback) => (lookupOptions && lookupOptions.all)
...
// `enforceMaxLength` (défaut true, comportement inchangé pour
// /api/url-knowledge/analyze) : le grounding web (lib/web-search-grounding.js,
// resolveWebSearchGrounding dans server.js) tronque de toute façon chaque
// source à WEB_SEARCH_EXCERPT_MAX_CHARS avant de l'injecter dans un prompt —
// rejeter en amont un article Wikipédia (souvent >50 000 caractères une fois
// extrait) n'y protège donc rien et privait Wikipédia du grounding malgré
// une sélection IA pourtant explicitement en sa faveur (trouvaille du
// 31/08/2026, cf. demande "privilégier Wikipédia").
function extractReadableContent(html, sourceUrl, options = {}) {
  ...
  try {
    ...
    if (options.enforceMaxLength !== false && text.length > TEXT_KNOWLEDGE_MAX_CHARS) throw new UrlKnowledgeError("content_too_long", "Cette page dépasse 50 000 caractères. Choisis un extrait avec Coller du texte.");
