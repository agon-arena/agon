...
// Plusieurs mécanismes possibles par jour, mais jamais un nombre fixe : le
...
const MAX_MECANISMES_PER_DAY = 3;
...
function validateMecanismeSociologiqueResponse(raw, topics) {
  ...
  if (!Array.isArray(raw.mecanismes) || raw.mecanismes.length < 1 || raw.mecanismes.length > MAX_MECANISMES_PER_DAY) {
    return { ok: false, reason: `mecanismes doit être un tableau de 1 à ${MAX_MECANISMES_PER_DAY} élément(s).` };
  }
