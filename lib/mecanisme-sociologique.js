...
const SOCIOLOGICAL_IMAGE_REPAIR_COOLDOWN_MS = 15 * 60 * 1000;
...
// recentConceptsNormalized (cf. lib/eclairages-recent-usage.js) contient les
// sociological_concept déjà publiés dans les RECENT_REPEAT_AVOIDANCE_DAYS
// derniers jours — filet de sécurité même si l'IA ignore la consigne du
// prompt.
function validateSingleMecanisme(raw, validTopicIds, knownUrls, recentConceptsNormalized) {
  ...
  if (recentConceptsNormalized && recentConceptsNormalized.has(normalizeIdentity(raw.sociological_concept))) {
    return { ok: false, reason: `sociological_concept déjà utilisé dans les ${RECENT_REPEAT_AVOIDANCE_DAYS} derniers jours.` };
  }
...
function validateMecanismeSociologiqueResponse(raw, topics, recentConceptsNormalized) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
  ...
  for (const item of raw.mecanismes) {
    const result = validateSingleMecanisme(item, validTopicIds, knownUrls, recentConceptsNormalized);
...
function createMecanismeSociologiqueService(deps) {
    ...
    try {
      ...
      if (image && image.imageUrl) {
        return {
          ...
          sociological_concept_image_checked_at: checkedAt
        };
      }
    } catch (err) {
  ...
  function mecanismeNeedsImageRepair(mecanisme) {
    if (!mecanisme || mecanisme.sociological_concept_image_url || (!String(mecanisme.sociologist_name || "").trim() && !String(mecanisme.sociological_concept || "").trim())) return false;
    const checkedAt = new Date(mecanisme.sociological_concept_image_checked_at || 0).getTime();
    return !Number.isFinite(checkedAt) || Date.now() - checkedAt >= SOCIOLOGICAL_IMAGE_REPAIR_COOLDOWN_MS;
  }

  async function repairPublishedConceptImages(row) {
    if (!row || row.status !== "published" || !Array.isArray(row.content && row.content.mecanismes)) return row;
    if (!row.content.mecanismes.some(mecanismeNeedsImageRepair)) return row;
    ...
    const repairPromise = (async () => {
      ...
      try {
        ...
        logger.error("[mecanisme-sociologique] récupération sujets pour réparation image :", err.message);
        return null;
      }
      const topicsById = new Map((Array.isArray(topics) ? topics : []).map((topic) => [String(topic.id), topic]));
      const mecanismes = [];
      for (const mecanisme of row.content.mecanismes) {
        mecanismes.push(mecanismeNeedsImageRepair(mecanisme)
          ? await attachConceptImageToOne(mecanisme, topicsById.get(String(mecanisme.current_topic_id)))
          : mecanisme);
      }
      const content = { ...row.content, mecanismes };
      const { error } = await supabase.from(TABLE).update({ content, updated_at: new Date().toISOString() }).eq("date", dateKey).eq("status", "published");
      if (error) {
        logger.error("[mecanisme-sociologique] écriture réparation image :", error.message);
        return null;
      }
      return content;
    })();
  ...
  async function runGeneration(dateKey) {
    ...
    if (!usableTopics.length) {
      if (!hadUsableTopicsBeforeExclusion) {
        ...
        logger.warn("[mecanisme-sociologique] aucun sujet publié pour l'instant, génération reportée.");
        return { status: "generating" };
      }
      const reason = "Tous les sujets exploitables du jour sont déjà couverts par le parallèle historique ou la pensée philosophique.";
    ...
    // Concepts déjà publiés dans les RECENT_REPEAT_AVOIDANCE_DAYS derniers
    // jours (cf. lib/eclairages-recent-usage.js) : transmis à l'IA (prompt)
    // et vérifiés à nouveau après coup (validateSingleMecanisme), pour ne
    // jamais reproposer le même concept trop vite.
    const { raw: recentConcepts, normalized: recentConceptsNormalized } = await fetchRecentEclairagesIdentities({
      supabase,
      table: TABLE,
      contentKey: "mecanismes",
      identityField: "sociological_concept",
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      const outcome = await attemptGenerationOnce(usableTopics, recentConcepts, recentConceptsNormalized);
      lastOutcome = outcome;
      if (outcome.ok) {
  ...
  async function attemptGenerationOnce(usableTopics, recentConcepts, recentConceptsNormalized) {
    let raw;
    try {
      const prompt = buildMecanismeSociologiquePrompt(usableTopics, recentConcepts);
    ...
    const validation = validateMecanismeSociologiqueResponse(raw, usableTopics, recentConceptsNormalized);
