...
const { fsrs, generatorParameters, createEmptyCard, forgetting_curve, Rating, State } = require("ts-fsrs");
...
const schedulerParams = generatorParameters({ request_retention: DEFAULT_REQUEST_RETENTION });
...
// Rétrivabilité actuelle (probabilité de s'en souvenir maintenant, 0 à 1) —
// dérivée en lecture seule de stability + temps écoulé depuis la dernière
// review (courbe d'oubli FSRS, cf. ts-fsrs forgetting_curve), jamais du
// scheduler lui-même. Utilisée pour des indicateurs affichés ("% d'ancrage"
// dans "Mes acquis", demande du 17/08/2026) plutôt que le crédit fixe par
// état (0.5 Learning/Relearning, 1 Review) qui ne tenait pas compte de
// l'oubli progressif d'une carte en retard de repasse.
// state : "shape" memory_item_fsrs_states (stability, lastReviewAt, state)
//   ou null/undefined si aucune review n'a encore eu lieu (New) -> 0.
// now : Date de référence (jamais tronquée à la journée, même convention que
//   reviewMemoryItem).
function computeRetrievability(state, now) {
  if (!state || state.state === "New" || !state.lastReviewAt) return 0;
  const stability = state.stability;
  if (typeof stability !== "number" || !Number.isFinite(stability) || stability <= 0) return 0;
  const elapsedDays = (now.getTime() - new Date(state.lastReviewAt).getTime()) / (24 * 60 * 60 * 1000);
  if (!Number.isFinite(elapsedDays) || elapsedDays < 0) return 1;
  return forgetting_curve(schedulerParams.w, elapsedDays, stability);
}
