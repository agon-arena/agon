...
// suggested_theme (demande du 14/09/2026, "pourquoi les icônes [...] pas mis
// aussi ? Là il y a l'icone autre ou non classé") : même thématique que
// title/reason, déjà calculée par le même appel IA (ai-fallback.js
// buildFallbackPrompt) mais jusqu'ici jetée à la porte ici — un sujet servi
// depuis ce stock de réserve (selectUnclaimedReadyTopics, jamais encore
// acquis par personne) n'avait donc aucune thématique résolvable côté
// frontend, contrairement aux recommandations catalogue et aux propositions
// IA équivalentes à un sujet déjà généré (corrigées le même jour). Colonne
// absente tant que data/migration-notion-quiz-pregeneration-queue-theme.sql
// n'a pas été appliquée : détection best-effort du message d'erreur Postgres
// et repli silencieux sans la colonne (même patron que upsertOpinionArticleRows,
// server.js), jamais un crash de l'enqueue pour ce seul champ additif.
async function enqueueProposedTopic({ supabase, title, findExistingMaster, source = "ai_fallback_proposal", suggestedTheme = null }) {
  ...
  let existingRow;
  {
    const { data, error: selectError } = await supabase
      .from(QUEUE_TABLE)
      .select("id, proposal_count, status, suggested_theme")
      .eq("normalized_key", normalizedKey)
      .maybeSingle();
    if (selectError) {
      if (!String(selectError.message || "").toLowerCase().includes("suggested_theme")) throw new Error(selectError.message);
  ...
  if (existingRow) {
    const updatePayload = { proposal_count: (existingRow.proposal_count || 1) + 1, last_proposed_at: now, updated_at: now };
    // Backfill opportuniste : ne remplace jamais une thématique déjà connue,
    // comble seulement un trou (première proposition avant ce correctif).
    if (themeColumnAvailable && suggestedTheme && !existingRow.suggested_theme) {
      updatePayload.suggested_theme = suggestedTheme;
    }
  ...
  if (themeColumnAvailable) insertPayload.suggested_theme = suggestedTheme || null;
  let { error: insertError } = await supabase.from(QUEUE_TABLE).insert(insertPayload);
  if (insertError && String(insertError.message || "").toLowerCase().includes("suggested_theme")) {
    console.warn("[notion-quiz-pregeneration] colonne suggested_theme absente : migration data/migration-notion-quiz-pregeneration-queue-theme.sql à appliquer.");
    delete insertPayload.suggested_theme;
    ({ error: insertError } = await supabase.from(QUEUE_TABLE).insert(insertPayload));
  }
...
async function selectUnclaimedReadyTopics({ supabase, limit }) {
  let { data: readyRows, error } = await supabase
    .from(QUEUE_TABLE)
    .select("id, title, master_slot, suggested_theme")
    .eq("status", "ready")
    .not("master_slot", "is", null)
    .order("updated_at", { ascending: false })
    .limit(Math.max(limit * 3, limit));
  if (error && String(error.message || "").toLowerCase().includes("suggested_theme")) {
    ({ data: readyRows, error } = await supabase
      .from(QUEUE_TABLE)
