...
async function queryWikipediaImage(lang, query, fetchImpl) {
  ...
  for (const page of sortPagesByRelevanceIndex(pages)) {
    ...
    const caption = await fetchWikipediaImageCaption(lang, page.title || "", page.pageimage || "", fetchImpl);
...
async function searchKnowledgeImage(query, options = {}) {
  ...
  try {
    ...
    if (commonsResult) {
      ...
      return { url: commonsResult.imageUrl, credit: commonsResult.credit, caption: null, pageUrl: commonsResult.pageUrl, source: "wikimedia-commons", width: commonsResult.width };
    }
  } catch (error) {
