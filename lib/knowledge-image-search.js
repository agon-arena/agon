...
// pas d'image de tête / ne partageait aucun mot avec la requête, sans jamais
// essayer le 2e ou 3e résultat pourtant souvent valide.
...
// Chrome/Safari). Le SVG a rejoint cette liste le 14/09/2026 ("annule cette
// règle [d'exclusion SVG] et prend aussi blason, drapeaux etc.") : jusque-là
// exclu pour écarter cartes/drapeaux/blasons/schémas génériques, mais ça
// écartait aussi les diagrammes/courbes (SVG quasi systématique sur
// Wikipedia pour ce type d'illustration, ex. la courbe de Laffer) qui SONT
// pourtant l'image la plus pertinente pour ces sujets précis.
const COMMONS_RENDERABLE_MIME_TYPES = new Set(["image/jpeg", "image/png", "image/webp", "image/gif", "image/svg+xml"]);
...
async function queryWikipediaImage(lang, query, fetchImpl) {
  ...
  for (const page of sortPagesByRelevanceIndex(pages)) {
    ...
    // Exclusion des images "de tête" en SVG (cartes, drapeaux, blasons,
    // schémas) retirée le 14/09/2026 ("annule cette règle et prend aussi
    // blason, drapeaux etc.") : elle écartait aussi les diagrammes/courbes
    // pourtant les plus pertinents pour certains sujets (ex. la courbe de
    // Laffer, en SVG sur Wikipedia comme la quasi-totalité des graphiques).
    ...
    // page.thumbnail.width en repli (constaté le 19/09/2026, "l'image de fond
    // derrière les QCM ne reprend jamais les images associées") : l'API
    // pageimages ne sert plus AUCUNE URL au format ".../NNNpx-fichier" depuis
    // son évolution vers le style "thumbnail_unscaled" (paramètres utm_*
    // plutôt que la largeur dans le chemin) — extractDeliveredWidthFromUrl
    // renvoie donc systématiquement null désormais, quelle que soit la
    // largeur réelle de l'image, ce qui faisait TOUJOURS échouer le seuil
    // MIN_BACKGROUND_IMAGE_WIDTH côté client (plus aucune image de fond
    // réelle, uniquement le repli générique). Vérifié en direct sur l'API :
    // thumbnail.width colle exactement à la largeur livrée dans ce nouveau
    // format (1851, 648, 848px mesurés sur de vraies pages), donc fiable ici
    // — contrairement à l'ancien souci documenté ci-dessus, qui concernait un
    // format d'URL différent, aujourd'hui disparu.
    const width = extractDeliveredWidthFromUrl(imageUrl) || (page.thumbnail && page.thumbnail.width) || null;
    return { imageUrl, pageUrl, pageTitle: page.title || null, width, caption };
  }
  return null;
}
