...
function buildQuestionsFromKnowledgePrompt(sourceIdField, sourceId, admittedKnowledge, levelInstruction, formatBlockLines, perKnowledgeCandidateCounts) {
...
function buildElementaryFichePrompt(subject, contextHint, elementaryKnowledge, levelConfig, groundingText = null) {
  ...
  lines.push(elementaryKnowledge.map((k, i) => `${i + 1}. ${k.knowledgeTarget}`).join("\n"));
  lines.push("");
  ...
  lines.push("- \"meta\" : une ligne courte de repères (dates, lieu, auteur...) si pertinent, sinon null.");
  lines.push(`- "sections" : ${sectionsRange} bloc(s) {"label": string ou null, "text": string} — ${lengthHint}`);
  ...
  lines.push("");
  lines.push('Réponds uniquement en JSON strict, sans aucun texte autour, sous cette forme exactement : {"sourceName":"...","meta":"..."|null,"sections":[{"label":"..."|null,"text":"..."}],"imageSearchQuery":"..."|null}');
  return lines.join("\n");
}
