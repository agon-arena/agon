...
} = require("./topic-identity-validation");
...
function buildFicheAndKnowledgeAdmissionPrompt(subject, contextHint, levelConfig, requireValidation, groundingText = null) {
  ...
  if (groundingText) {
    ...
    // "supprimé dans les années 1920", formulation catégorique reprise telle
    // quelle par un modèle qui n'avait aucun moyen de savoir qu'une source
    // muséale majeure restait bien plus prudente sur le même point). Chaque
    // source ci-dessous porte déjà une étiquette A+/A/B/C (cf.
    // lib/web-search-grounding.js buildGroundingText,
    // lib/source-scoring.js classifyDomainAuthorityTier) — ce paragraphe
    // explicite simplement comment l'exploiter, sans nouvel appel IA/réseau.
    lines.push("Chaque source ci-dessous porte une étiquette de fiabilité indicative entre parenthèses : A+ (institutionnel/gouvernemental/académique/muséal), A (encyclopédie ou référence académique de premier plan), B (source secondaire sérieuse) ou C (encyclopédie généraliste collaborative, ex. Wikipédia). En cas de divergence, de contradiction ou de simple différence de précision/prudence entre les sources sur un même point, privilégie TOUJOURS la formulation de la source au niveau le plus élevé — une source de niveau C sert de complément utile mais ne doit jamais l'emporter sur une source A+/A mieux informée ou plus prudente ; ne synthétise jamais artificiellement les deux comme si elles avaient la même autorité.");
  ...
  if (requireValidation) {
    ...
    lines.push("");
    lines.push("Étape 2 : si le sujet est valide, vérifie ensuite ceci avant de rédiger quoi que ce soit :");
  }
  // topicValidation (demande du 06/09/2026, incident "Baudouin de Hainaut") :
  // même bloc partagé que lib/notion-quiz-curriculum.js (jamais reformulé
  // différemment d'un pipeline à l'autre) — cf. lib/topic-identity-validation.js.
  // Intégré à CET appel, qui lit déjà groundingText avant de rédiger la fiche
  // (jamais un second appel IA dédié).
  lines.push(...buildTopicValidationInstructions(subject, !!groundingText));
  lines.push("");
  if (requireValidation) {
    lines.push("Étape 3 : si le sujet est valide ET non ambigu, rédige :");
  } else {
    lines.push("Rédige :");
  }
  ...
  lines.push("   Sois particulièrement prudent avec \"premier\", \"dernier\", \"seul\", \"unique\", \"toujours\", \"jamais\", \"principal\", \"exactement\", \"officiel\"/\"officiellement\", \"interdit\", \"supprimé\", \"définitivement\", \"cause\", \"provoque\", \"est responsable de\" : reformule plus prudemment si besoin, ou écarte le fait."
    + (groundingText ? " Redouble de prudence quand une telle affirmation forte ne provient QUE d'une source de niveau C (Wikipédia) parmi celles ci-dessus, sans aucune corroboration par une source A+/A : préfère alors une formulation mesurée et nuancée plutôt que de la reprendre telle quelle comme un fait établi (ex. \"occupe une place importante dans\" plutôt que \"est l'art officiel de\")." : ""));
  ...
  lines.push("");
  lines.push(`Réponds uniquement en JSON strict, sans aucun texte autour${requireValidation ? `, sous l'une de ces trois formes exactement :\n- Sujet refusé : {"valid":false,"reason":"..."}\n- Sujet ambigu (cf. plus haut) : ${TOPIC_VALIDATION_AMBIGUOUS_JSON_EXAMPLE}\n- Sujet accepté et non ambigu : {"valid":true,"topicValidation":{"status":"valid","normalizedTopic":"..."|null},"sourceName":"...","meta":"..."|null,"sections":[{"label":"..."|null,"text":"..."}],"imageSearchQuery":"..."|null,"knowledge":[{"fact":"...","importance":"high|medium|low","certainty":"high|medium|low","sequential":true|false,"clearBoundary":true|false}]}` : `, sous l'une de ces deux formes exactement :\n- Sujet ambigu (cf. plus haut) : ${TOPIC_VALIDATION_AMBIGUOUS_JSON_EXAMPLE}\n- Sujet non ambigu : {"topicValidation":{"status":"valid","normalizedTopic":"..."|null},"sourceName":"...","meta":"..."|null,"sections":[{"label":"..."|null,"text":"..."}],"imageSearchQuery":"..."|null,"knowledge":[{"fact":"...","importance":"high|medium|low","certainty":"high|medium|low","sequential":true|false,"clearBoundary":true|false}]}`}`);
  return lines.join("\n");
}
...
function buildKnowledgeVerificationPrompt(candidates, subjectLabel, groundingText = null) {
  ...
  return [
    ...
    "- elle risque de devenir rapidement obsolète (sauf si la date/le caractère temporel est précisément ce qui doit être retenu)."
      + (groundingText ? " ;\n- elle n'est PAS clairement soutenue par les sources ci-dessous, même si elle te semble par ailleurs plausible de mémoire ;\n- sa formulation reste absolue ou catégorique (\"le premier\", \"le seul\", \"officiellement\", \"interdit\", \"supprimé\"...) alors qu'elle n'est étayée, parmi les sources ci-dessous (chacune porte une étiquette A+/A/B/C), que par une source de niveau C (Wikipédia), sans aucune corroboration par une source A+/A." : ""),
    "",
...
function buildElementaryFichePrompt(subject, contextHint, elementaryKnowledge, levelConfig, groundingText = null) {
  ...
  if (groundingText) {
    ...
    lines.push(groundingText);
    // Garde-fou (demande du 06/09/2026, incident "Baudouin de Hainaut") :
    // l'identité du sujet a déjà été validée en amont (curriculum_generation,
    // cf. resolveProgressiveCurriculum côté server.js), mais ce prompt reçoit
    // encore le texte BRUT et complet des sources — jamais un second appel IA
    // ici, seulement ce rappel.
    lines.push(buildSourceMixingGuardRailLine());
  }
  lines.push("");
...
function buildProgressiveContinuationFichePrompt(subject, contextHint, levelKnowledge, levelConfig, groundingText, priorSectionsText, levelLabel) {
  ...
  if (groundingText) {
    ...
    lines.push("Voici de VRAIES sources web trouvées sur ce sujet — base la fiche PRINCIPALEMENT dessus. N'utilise ta mémoire que pour du contexte général ou des connaissances de base incontestables — jamais pour un fait précis absent de ces sources.");
    lines.push(groundingText);
    // Garde-fou (demande du 06/09/2026, incident "Baudouin de Hainaut") : cf.
    // le même commentaire dans buildElementaryFichePrompt ci-dessus.
    lines.push(buildSourceMixingGuardRailLine());
  }
  lines.push("");
