...
function buildKnowledgeAdmissionPrompt(sourceText, levelInstruction) {
  return [
    ...
    "- \"sequential\" (booléen) : true UNIQUEMENT si ce fait décrit une vraie séquence objective et établie (étapes historiques attestées, processus réel, ordre chronologique, procédure structurée), qui pourrait honnêtement se tester en \"remets dans l'ordre\" — false dans tous les autres cas, y compris pour un concept ou une théorie qu'on pourrait artificiellement présenter comme une suite d'étapes.",
...
function buildQuestionsFromKnowledgePrompt(sourceIdField, sourceId, admittedKnowledge, levelInstruction, formatBlockLines) {
  const knowledgeLines = admittedKnowledge
    .map((k, i) => `${i + 1}. ${k.fact} ${k.sequential ? "[séquence réelle : \"ordre\" envisageable]" : "[PAS de séquence réelle : jamais \"ordre\"]"} ${k.clearBoundary ? "[frontière nette : \"intrus\" envisageable]" : "[frontière floue : jamais \"intrus\"]"}`)
    .join("\n");
  return [
...
// candidates en un seul appel ── Ce pipeline n'a JAMAIS de texte source
// externe — contrairement à Éclairages/Histoire (buildKnowledgeAdmissionPrompt),
// la fiche elle-même est rédigée par le modèle à partir du seul nom du
// sujet. C'est le pipeline le plus fragile identifié par l'audit (aucun
// grounding externe). Cette fonction ne rédige donc QUE la fiche + une
// liste de connaissances CANDIDATES qu'elle en extrait (jamais de questions
// ici) : l'admission définitive passe ensuite par une SECONDE passe IA
// indépendante et conservatrice (cf. buildKnowledgeVerificationPrompt)
// avant que buildQuestionsFromKnowledgePrompt ne les transforme en
// questions.
// Important, à ne jamais perdre de vue : cette seconde passe améliore la
// robustesse (un second regard, plus prudent, sur les mêmes candidats) mais
// ne constitue PAS un grounding externe réel — rien ici ne vérifie ces
// faits contre une source indépendante du modèle lui-même.
function buildFicheAndKnowledgeAdmissionPrompt(subject, contextHint, levelConfig, requireValidation) {
  const { instruction, sectionsRange, lengthHint } = levelConfig;
  ...
  lines.push("- \"sequential\" (booléen) : true UNIQUEMENT si ce fait décrit une vraie séquence objective et établie (étapes attestées, processus réel, chronologie) — false sinon, y compris pour un concept qu'on pourrait artificiellement présenter comme une suite d'étapes.");
...
// commentaires : cette passe ne prouve toujours pas la véracité du fait par
// une source externe — c'est un second jugement IA, pas un fait-checking.
function buildKnowledgeVerificationPrompt(candidates, subjectLabel) {
