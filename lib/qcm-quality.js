...
function questionOptionsSignature(question) {
  const values = Array.isArray(question?.options) ? question.options.map(normalizeComparisonText).sort() : [];
  return values.join("|");
}
...
function validateQuestionBatchQuality(questions, options = {}) {
  ...
  for (let i = 0; i < decisions.length; i += 1) {
    for (let j = i + 1; j < decisions.length; j += 1) {
      const left = decisions[i].question;
      const right = decisions[j].question;
