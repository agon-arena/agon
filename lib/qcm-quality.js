...
// paragraphe/à la fiche/à la source/au document ayant servi à la générer") :
// le grounding (fiche, sources, evidence_text) sert uniquement à ANCRER la
// connaissance en interne — il ne doit jamais transparaître dans l'énoncé
// final ("d'après le texte...", "selon la source...", "le document
// indique..."). Contrôle purement lexical sur le texte NORMALISÉ
// (normalizeComparisonText ci-dessus a déjà neutralisé casse, accents et
// apostrophes/guillemets typographiques) — d'où des motifs écrits ici SANS
// accent et avec une apostrophe droite uniquement. Construit comme deux
// familles de motifs à deux mots minimum (jamais un mot isolé comme
// "texte"/"source"/"document"/"passage", qui sont des sujets de question
// parfaitement légitimes en histoire/culture générale — ex. "Quel document
// Justinien fait-il rédiger ?" ne doit jamais être rejeté) :
// (a) une locution d'introduction ("d'après"/"selon"/"dans" + déterminant +
//     texte/source/document/passage/informations fournies) ;
// (b) le support employé comme SUJET d'un verbe méta-référentiel
//     (indique/explique/attribue/précise/mentionne).
// Couvre au minimum les 17 motifs requis par l'audit du 04/09/2026 (cf.
// test/qcm-quality.test.js) — la combinatoire verbe×support ci-dessous
// ajoute quelques variantes symétriques de la même famille (ex. "la source
// précise", jamais explicitement listée mais strictement équivalente à "le
// texte précise") plutôt que de les lister une à une.
const SOURCE_REFERENCE_INTRO_RE = /\b(?:d'apres|selon|dans)\s+(?:le|la|les|ce|cette|cet)\s+(?:texte|source|document|passage|informations fournies)\b/;
const SOURCE_REFERENCE_SUBJECT_RE = /\b(?:le texte|la source|le document|le passage)\s+(?:indique|indiquent|explique|expliquent|attribue|attribuent|precise|precisent|mentionne|mentionnent)\b/;

function hasSourceReferenceWording(value) {
  const normalized = normalizeComparisonText(value);
  if (!normalized) return false;
  return SOURCE_REFERENCE_INTRO_RE.test(normalized) || SOURCE_REFERENCE_SUBJECT_RE.test(normalized);
}
...
function validateQuestionQuality(item, options = {}) {
  ...
  if (hasSourceReferenceWording(question)) reasons.push(reason("SOURCE_REFERENCE_WORDING", "La question fait référence au support source (« d'après le texte », « selon la source »...) au lieu d'interroger directement la connaissance — le grounding doit rester invisible pour l'utilisateur.", ["question"]));
  ...
  while (candidates.length) {
    ...
    if (options.groundingSources) {
      for (const decision of cycleDecisions) {
        ...
        // hasEvidenceOverride (Phase 2.1, correctif du 04/09/2026) : prédicat
        // optionnel, absent pour tout appelant existant (comportement
        // STRICTEMENT inchangé) — fourni UNIQUEMENT par le pipeline
        // progressif pour les questions dont supporting_claim a été forcé
        // à un evidence_text déjà vérifié déterministiquement en amont (cf.
        // lib/question-grounding-validation.js pour le diagnostic complet).
        const skipAnswerAlignmentCheck = typeof options.hasEvidenceOverride === "function" && options.hasEvidenceOverride(decision.question);
        const grounding = validateQuestionGrounding(decision.question, options.groundingSources, { skipAnswerAlignmentCheck });
        if (!grounding.ok) {
    ...
    // cycleDeterministicRejected (observabilité, 03/09/2026, audit réel "Les
    // oiseaux migrateurs") : snapshot de `rejected` AVANT toute critique
    // sémantique — sert uniquement à distinguer, dans le log
    // [qcm-quality-cycle], les rejets déterministes des rejets sémantiques
    // de CE cycle (les entrées poussées ensuite par la critique, plus bas,
    // ne sont jamais dans ce snapshot). Jamais utilisé pour une décision.
    const cycleDeterministicRejected = rejected.slice();
    ...
    // onInitialBatchAccepted (sur-génération initiale du bloc élémentaire,
    // 03/09/2026, cf. server.js generateElementaryBlock) : optionnel, absent
    // pour tout appelant existant — AUCUN changement de comportement
    // ailleurs. Appelé UNE SEULE fois, juste après que le tout premier lot
    // (avant toute régénération, `cycles` vaut encore 0 à ce point) a été
    // validé et comptabilisé ci-dessus — permet à l'appelant de mesurer
    // combien de connaissances distinctes étaient déjà couvertes par ce
    // seul premier appel, jamais une décision (best-effort et silencieux,
    // même philosophie que options.onCycle).
    if (cycles === 0 && typeof options.onInitialBatchAccepted === "function") {
      try {
        options.onInitialBatchAccepted(accepted.slice());
      } catch (_) {
    ...
    // mesure, jamais une décision — ce callback lui-même ne déclenche aucun
    // early stop (MIN_MASTER_QUESTIONS reste, pour le master legacy, un
    // seuil d'acceptabilité, jamais une cible d'arrêt : cf. options.
    // earlyStopAtAccepted ci-dessous pour le SEUL cas qui en a un, opt-in).
    // Best-effort et silencieux par construction (même philosophie que
    // recordAiUsage/imagePromise ailleurs dans le pipeline) : une panne de
    // télémétrie ne doit jamais interrompre ni modifier une génération réelle.
    if (typeof options.onCycle === "function") {
      try {
        // deterministicReasonCounts/semanticReasonCounts (observabilité,
        // 03/09/2026) : codes agrégés de CE cycle uniquement, jamais de
        // texte de question ni de contenu utilisateur — cycleDeterministic
        // Rejected est le snapshot pris avant toute critique sémantique
        // (ci-dessus), le reste de `rejected` (poussé par la critique plus
        // haut) est donc, par construction, uniquement sémantique.
        options.onCycle({
          cycleIndex: cycles,
          questionsIn,
          ...
          deterministicReasonCounts: aggregateReasonCodes(cycleDeterministicRejected),
          semanticReasonCounts: aggregateReasonCodes(rejected.slice(cycleDeterministicRejected.length))
        });
      } catch (_) {
        // jamais bloquant ni remonté — cf. commentaire ci-dessus
      }
    ...
    // optionnel, absent (`undefined`) pour tout appelant existant — donc
    // AUCUN changement de comportement pour le master legacy ou tout autre
    // appelant, qui continue de régénérer jusqu'à cycles>=maxRetries comme
    // avant. Quand fourni, arrête la boucle dès que le compte considéré
    // atteint ce seuil — n'assouplit JAMAIS un critère de validation : une
    // question rejetée à CE cycle reste rejetée, seule une régénération
    // FUTURE devient inutile.
    // earlyStopCountFn (sur-génération initiale, 03/09/2026) : optionnel,
    // fonction (accepted) => nombre. Absent (comportement de tout appelant
    // existant), le compte reste `accepted.length` tel quel — inchangé au
    // caractère près. Fourni par generateElementaryBlock pour compter les
    // connaissances DISTINCTES couvertes (jamais le nombre brut de
    // questions acceptées, qui peut dépasser le nombre de connaissances
    // lorsque plusieurs candidats couvrent la même connaissance).
    const earlyStopCount = typeof options.earlyStopCountFn === "function" ? options.earlyStopCountFn(accepted) : accepted.length;
    // filterRejectedForRegeneration (sur-génération initiale, 03/09/2026) :
    // optionnel, fonction (rejected, accepted) => rejected filtré. Absent
    // (comportement de tout appelant existant), regenerationTargets reste
    // strictement `rejected` — inchangé au caractère près. Fourni par
    // generateElementaryBlock pour ne jamais dépenser un cycle de
    // régénération sur une connaissance déjà couverte par une autre
    // question acceptée (cf. son commentaire pour le détail).
    const regenerationTargets = typeof options.filterRejectedForRegeneration === "function"
      ? options.filterRejectedForRegeneration(rejected, accepted)
      : rejected;
    if (!regenerationTargets.length || cycles >= maxRetries || typeof regenerate !== "function"
      || (options.earlyStopAtAccepted != null && earlyStopCount >= options.earlyStopAtAccepted)) break;
    ...
    candidates = await regenerate({ rejected: regenerationTargets, accepted: accepted.slice(), attempt: cycles });
