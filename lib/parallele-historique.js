...
// Plusieurs parallèles possibles par jour, mais jamais un nombre fixe : le
// prompt (cf. prompts/parallele-historique.js) instruit explicitement de
// n'en publier qu'autant que de sujets s'y prêtant vraiment (1 à 3), jamais
// pour atteindre un quota. Cette constante n'est qu'un plafond de sécurité.
const MAX_PARALLELS_PER_DAY = 3;
...
function validateParalleleHistoriqueResponse(raw, topics) {
  ...
  if (!Array.isArray(raw.parallels) || raw.parallels.length < 1 || raw.parallels.length > MAX_PARALLELS_PER_DAY) {
    return { ok: false, reason: `parallels doit être un tableau de 1 à ${MAX_PARALLELS_PER_DAY} élément(s).` };
  }
...
function createParalleleHistoriqueService(deps) {
  ...
  async function runGeneration(dateKey) {
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      ...
      lastOutcome = outcome;
      if (outcome.ok) {
        const enrichedData = await attachHistoricalEventImages(outcome.data);
  ...
  // Ajoute une image réelle du précédent historique (Wikipedia) à UN
  // parallèle déjà validé. Ne fait jamais échouer la publication : en cas
  // d'erreur ou d'absence de résultat, ce parallèle est publié sans image.
  async function attachHistoricalEventImageToOne(parallel) {
    try {
      const image = await fetchHistoricalEventImage(parallel.historical_event_title, parallel.historical_context);
      if (image && image.imageUrl) {
        return { ...parallel, historical_event_image_url: image.imageUrl, historical_event_image_page_url: image.pageUrl || null };
      }
    } catch (err) {
      logger.error("[parallele-historique] recherche image Wikipedia :", err.message);
    }
    return { ...parallel, historical_event_image_url: null, historical_event_image_page_url: null };
  }
  ...
  async function attachHistoricalEventImages(data) {
    const enrichedParallels = [];
    for (const parallel of data.parallels) {
      enrichedParallels.push(await attachHistoricalEventImageToOne(parallel));
    }
