...
// recentEventsNormalized (cf. lib/eclairages-recent-usage.js) contient les
// historical_event_title déjà publiés dans les RECENT_REPEAT_AVOIDANCE_DAYS
// derniers jours — filet de sécurité même si l'IA ignore la consigne du
// prompt.
function validateSingleParallel(raw, validTopicIds, knownUrls, recentEventsNormalized) {
  ...
  if (recentEventsNormalized && recentEventsNormalized.has(normalizeIdentity(raw.historical_event_title))) {
    return { ok: false, reason: `historical_event_title déjà utilisé dans les ${RECENT_REPEAT_AVOIDANCE_DAYS} derniers jours.` };
  }
...
function validateParalleleHistoriqueResponse(raw, topics, recentEventsNormalized) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
  ...
  for (const item of raw.parallels) {
    const result = validateSingleParallel(item, validTopicIds, knownUrls, recentEventsNormalized);
...
function createParalleleHistoriqueService(deps) {
  ...
  async function runGeneration(dateKey) {
    ...
    // Précédents historiques déjà publiés dans les RECENT_REPEAT_AVOIDANCE_DAYS
    // derniers jours (cf. lib/eclairages-recent-usage.js) : transmis à l'IA
    // (prompt) et vérifiés à nouveau après coup (validateSingleParallel),
    // pour ne jamais reproposer le même événement trop vite.
    const { raw: recentEvents, normalized: recentEventsNormalized } = await fetchRecentEclairagesIdentities({
      supabase,
      table: TABLE,
      contentKey: "parallels",
      identityField: "historical_event_title",
    ...
    for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
      const outcome = await attemptGenerationOnce(usableTopics, recentEvents, recentEventsNormalized);
      lastOutcome = outcome;
      if (outcome.ok) {
  ...
  async function attemptGenerationOnce(usableTopics, recentEvents, recentEventsNormalized) {
    let raw;
    try {
      const prompt = buildParalleleHistoriquePrompt(usableTopics, recentEvents);
    ...
    const validation = validateParalleleHistoriqueResponse(raw, usableTopics, recentEventsNormalized);
