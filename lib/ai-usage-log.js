...
// gpt-5.6-luna (ajouté le 01/09/2026, cf. server.js OPENAI_DAILY_QUIZ_NARRATIVE_MODEL/
// OPENAI_DAILY_QUIZ_CRITIC_MODEL) : tarif du palier standard (< 272K tokens
// d'input), vérifié le 01/09/2026 sur developers.openai.com/api/docs/pricing.
// Mnoria ne dépasse jamais ce seuil (fiches/QCM très loin de 272K tokens) —
// le palier "long context" de ce modèle (2x input, 1.5x output au-delà de
// 272K) n'est donc volontairement PAS modélisé ici, cf. rapport
// d'instrumentation (risque résiduel documenté, pas un oubli).
...
// Remise Batch OpenAI (chantier pré-génération, 07/09/2026) : politique
// tarifaire PUBLIQUE et officielle d'OpenAI pour l'API Batch — -50% sur
// TOUS les tokens (input, input caché, output), pour TOUS les modèles,
// sans exception ni palier. Documentée sur developers.openai.com/api/docs/pricing,
// jamais un chiffre mesuré nous-mêmes via une facture réelle (nous n'avons
// pas accès à la facturation OpenAI depuis ce code) — mais jamais non plus
// un chiffre inventé : c'est la remise qu'OpenAI applique effectivement, la
// même pour un appel synchrone et son équivalent Batch tant que le modèle et
// les tokens sont identiques (vérifié §4 du canari : payload strictement
// équivalent entre les deux chemins).
const OPENAI_BATCH_DISCOUNT = 0.5;
...
function estimateCostUsd(model, { inputTokens, cachedTokens = 0, outputTokens, isBatch = false } = {}) {
  ...
  // isBatch (07/09/2026) : mêmes tokens, même modèle, seule la remise Batch
  // officielle change — jamais un second barème de prix inventé.
  const finalCost = isBatch ? cost * OPENAI_BATCH_DISCOUNT : cost;
  ...
  return Math.round(finalCost * 1e8) / 1e8;
}
...
async function recordAiUsage(supabase, {
  ...
  outputTokens = null,
  cachedTokens = null,
  // generationId (01/09/2026) : identifiant commun à tous les appels IA
  // d'une même génération QCM (cf. server.js — même "id"/"sourceId" déjà
  // propagé dans tout le pipeline notion-quiz). Optionnel et rétrocompatible :
  // absent pour tout appelant hors pipeline QCM, colonne NULL en base.
  generationId = null,
  latencyMs = null,
  success,
  error = null,
  // isBatch/batchId (chantier pré-génération, 07/09/2026) : distingue un
  // appel facturé au tarif Batch officiel (-50%, cf. OPENAI_BATCH_DISCOUNT)
  // d'un appel synchrone classique — batchId permet de retrouver le Batch
  // OpenAI d'origine pour un audit précis, sans jamais construire de système
  // de métriques agrégées séparé (une requête SQL sur is_batch suffit).
  isBatch = false,
  batchId = null
} = {}) {
  try {
    ...
    const estimatedCostUsd = (typeof inputTokens === 'number' && typeof outputTokens === 'number')
      ? estimateCostUsd(model, { inputTokens, cachedTokens: cachedTokens || 0, outputTokens, isBatch })
      : null;
