...
RÈGLE DE CORRECTION PAR DÉFAUT POUR CE BARÈME UTILISATEUR

- Corrige strictement critère par critère selon le barème fourni.
- N'exige jamais plus que ce qui est écrit dans le barème.
- Si un élément attendu est présent, même formulé simplement, valide-le.
- Ne retire des points que si un élément précis du barème est absent, faux ou vraiment insuffisant.
- Les commentaires doivent toujours être cohérents avec les points attribués.
...
async function generateAnalysisJson(payload, callOpenAI) {
  ...
  const [p1RawA, p1RawB, grid] = await Promise.all([
    ...
    runP1A ? callOpenAI([{ role: 'user', content: buildP1(question, labelA, argsA) }], { temperature: 0.1 }) : Promise.resolve(null),
    runP1B ? callOpenAI([{ role: 'user', content: buildP1(question, labelB, argsB) }], { temperature: 0.1 }) : Promise.resolve(null),
