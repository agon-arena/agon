...
- 11–15 : mécanisme, exemple, référence ou conséquence assez clair.
...
- exemple_precis_present : l'argument donne-t-il un exemple précis (pays, chiffre, cas réel, période historique) ?
...
Précision / mécanisme concret (critère 4) ≤ 10/20 si :
...
Qualité du raisonnement (critère 3) ≤ 21/30 si :
texte_clair_mais_general = oui OU affirmation_sans_demonstration = oui

Nuance (critère 5) ≤ 5/10 si :
objection_precise_presente = non OU objection_developpee = non OU formulation_moderee_sans_nuance_reelle = oui
...
- Si texte_clair_mais_general = oui → reasoning ≤ 21. Si supérieur, corrige à 21.
- Si mecanisme_concret_present = non ET seuil_present = non ET modalite_presente = non ET exemple_precis_present = non ET effet_attendu_explique = non → precision ≤ 10. Si supérieur, corrige à 10.
- Si objection_precise_presente = non OU objection_developpee = non OU formulation_moderee_sans_nuance_reelle = oui → nuance ≤ 5. Si supérieur, corrige à 5.
  ...
  async function scoreOne(arg, camp) {
    ...
    if (!isCustom && scoresDetail && Object.keys(scoresDetail).length > 0) {
      ...
      if (capNuance === null && diagnosticData) {
        if (isNon('objection_precise_presente') || isNon('objection_developpee') ||
