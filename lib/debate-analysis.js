...
async function buildScoringGrid({ isOpen, question, content, evaluationAxis, correctionStrictness, previousGrid }, callOpenAI) {
  ...
  try {
    ...
    }) }], { temperature: 0, model: 'gpt-4o', feature: 'debate_grid' });
...
async function scoreP2WithConsistency({ prompt, isCustom: isCustomFlag, grid, callOpenAI, context }) {
  ...
  const rawA = await callOpenAI(messages, { temperature: 0, feature: 'debate_p2' });
  ...
  const rawB = await callOpenAI(messages, { temperature: 0, feature: 'debate_p2' });
  ...
  const rawC = await callOpenAI(messages, { temperature: 0, model: 'gpt-4o', feature: 'debate_arbitration' });
...
async function generateAnalysisJson(payload, callOpenAI, { forceRescore = false, fetchContent = null } = {}) {
  ...
  const [p1RawA, p1RawB, grid] = await Promise.all([
    ...
    runP1A ? callOpenAI([{ role: 'user', content: buildP1(question, labelA, argsA) }], { temperature: 0, feature: 'debate_p1' }) : Promise.resolve(null),
    runP1B ? callOpenAI([{ role: 'user', content: buildP1(question, labelB, argsB) }], { temperature: 0, feature: 'debate_p1' }) : Promise.resolve(null),
  ...
  async function scoreOne(arg, camp) {
    ...
    if (hasUrl && !isCustom) {
      ...
      ], { temperature: 0, feature: 'debate_p3' });
    }
  ...
  if (!isOpen) {
    const p4Raw   = await callOpenAI([{ role: 'user', content: buildP4(question, labelA, labelB, verdict, scoredA, scoredB) }], { feature: 'debate_p4' });
