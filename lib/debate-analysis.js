...
- 16–20 : fait précis (chiffre, seuil, donnée datée) cité dans le texte ET source dédiée fournie par l'auteur. La source est évaluée séparément — ne pas la noter ici — mais sa présence autorise ce palier pour une donnée précise.
...
- texte_clair_mais_general : le texte est-il compréhensible mais sans développement substantiel (thèse lisible, mais "comment" et "pourquoi" restent vagues) ?
...
Précision / mécanisme concret (critère 4) ≤ 12/20 si :
...
- Argument sans mécanisme concret, sans exemple, sans conséquence détaillée → total ≤ 70/100.
- Argument sans vraie nuance (au sens défini ci-dessus) → total ≤ 70/100.
...
- Si mecanisme_concret_present = non ET seuil_present = non ET modalite_presente = non ET exemple_precis_present = non ET effet_attendu_explique = non → precision ≤ 12. Si supérieur, corrige à 12.
...
- Contribution sans mécanisme, sans exemple, sans conséquence détaillée → total ≤ 70/100.
- Contribution sans vraie nuance (au sens défini ci-dessus) → total ≤ 70/100.
...
const SCORING_RUBRIC_VERSION = 5; // v5 : nuance redéfinie strictement (une idée qui va dans le sens de la thèse ne compte pas), plafonds absolus ajoutés, distinction affirmation/argument/raisonnement dans le critère reasoning
...
function buildP2(question, camp, arg, grid) {
  const hasSource = Boolean(String(arg.source_url || '').trim());
  return fill(PROMPT2, {
    question, camp,
    ...
    source_indicator: hasSource
      ? 'Source dédiée fournie par l\'auteur (évaluée séparément — ne pas l\'évaluer ici).'
      : 'Aucune source dédiée fournie.'
  });
}
...
async function generateAnalysisJson(payload, callOpenAI, { forceRescore = false } = {}) {
  ...
  async function scoreOne(arg, camp) {
    ...
    const [p2, p3Raw] = await Promise.all([
