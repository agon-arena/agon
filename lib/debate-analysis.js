...
Ne pas confondre mention du thème et traitement réel de la question. Un texte qui se contente d'un slogan, d'une opinion générale ou d'une affirmation de principe sans répondre précisément à la question ne doit pas dépasser 12/20, même s'il utilise les mots du sujet. Pour dépasser 15/20, le texte doit répondre précisément à la question posée, pas seulement évoquer le thème général.
...
Ne confonds pas affirmation, argument et raisonnement construit :
— Affirmation : "La vérification est importante." → simple déclaration, aucun développement. Note ≤ 14/30.
— Argument : "La désinformation est dangereuse, donc il faut vérifier." → lien cause/effet, mais sans mécanisme détaillé. Note ≤ 21/30.
— Raisonnement construit : explique le mécanisme précis — comment, par qui, avec quelle conséquence concrète ou quel exemple. Note ≥ 22/30 possible.

Ne récompense PAS les grandes formules rhétoriques ni les mots-clés moraux ou politiques ("liberté", "démocratie", "censure", "confiance", "dignité") quand ils ne sont pas étayés par un développement. La force rhétorique d'une formule n'est pas un raisonnement.

Ne récompense PAS non plus une position dite "équilibrée", "réformiste" ou "pragmatique" si elle ne développe aucun mécanisme précis. Formuler une "réforme plus équilibrée", un "juste milieu" ou une "solution progressive" sans expliquer comment, pour qui, avec quels effets — reste un argument vague, quelle que soit son apparence de modération. La modération rhétorique n'est pas un argument.
...
Une référence théorique pertinente et utilisée comme mécanisme explicatif (auteur reconnu, concept établi — capital culturel, effet Matthieu, trappe à pauvreté, etc. — donnée empirique) compte comme précision : elle ancre l'argument dans une logique démontrée. Traite-la comme un mécanisme si elle est liée à la démonstration, pas comme un simple ornement.

Une alternative générale du type "il faut une réforme équilibrée" ou "une approche progressive" ne compte PAS comme mécanisme concret si elle ne précise ni seuil, ni modalité, ni exemple, ni effet attendu. La modération de la proposition ne remplace pas sa précision.

Dans la limite de 600 caractères, une phrase qui précise clairement une règle, une exception, une procédure, un acteur compétent ou un effet juridique/pratique doit compter comme mécanisme concret, même si elle n'est pas longuement développée.

- 0–5 : idée très générale, slogan ou affirmation sans précision.
- 6–10 : début de précision, mais mécanisme encore vague. Aucune référence nommée, aucun exemple précis.
- 11–15 : mécanisme, exemple, référence ou conséquence assez clair. Un auteur reconnu nommé et utilisé comme mécanisme d'explication (ex : "capital culturel de Bourdieu", "effet Matthieu") atteint ce palier même sans chiffre associé. Un chiffre ou fait précis cité sans source dédiée reste dans ce palier (invérifiable).
- 16–20 : argument très précis : seuil chiffré, modalité concrète ou effet attendu clairement expliqué — on peut le contester point par point sans avoir à le reformuler.
...
Précision / mécanisme concret (critère 4) ≤ 14/20 si :
mecanisme_concret_present = non ET seuil_present = non ET modalite_presente = non ET exemple_precis_present = non ET effet_attendu_explique = non

Qualité du raisonnement (critère 3) ≤ 24/30 si :
texte_clair_mais_general = oui OU affirmation_sans_demonstration = oui
...
- Argument composé principalement d'opinions claires mais peu argumentées → total ≤ 60/100.
...
- Opinion vague → 35–55
- Argument clair mais peu développé → 55–65
- Argument correct avec mécanisme ou exemple → 65–75
- Argument solide, précis, nuancé → 75–85
- Argument excellent (+ source fiable) → 85–100

{{strictness_note}}

FOND ET EXPRESSION SONT DEUX DIMENSIONS INDÉPENDANTES

Les critères 3 (raisonnement) et 4 (précision) évaluent le FOND : la solidité logique, la présence d'un mécanisme, d'une référence ou d'un exemple. Ces critères ne doivent PAS être minorés en raison de fautes d'orthographe, d'une syntaxe maladroite ou d'un texte inachevé.

Les critères 2 (clarté de la thèse) et 6 (ton) peuvent être affectés par la qualité formelle de l'expression.

Conséquence directe : un argument contenant des fautes et une fin incomplète peut légitimement obtenir 22/30 au raisonnement et 14/20 à la précision si son contenu l'impose. À l'inverse, un argument bien rédigé mais vague obtient ≤ 14/30 au raisonnement et ≤ 10/20 à la précision. Ne laisse pas la propreté formelle compenser l'absence de fond.
...
- Si texte_clair_mais_general = oui → reasoning ≤ 24. Si supérieur, corrige à 24.
- Si mecanisme_concret_present = non ET seuil_present = non ET modalite_presente = non ET exemple_precis_present = non ET effet_attendu_explique = non → precision ≤ 14. Si supérieur, corrige à 14.
...
const SCORING_RUBRIC_VERSION = 6; // v6 : barème assoupli pour les formats 600 caractères (precision cap 14, plafonds nuance/mécanisme moins punitifs)
...
async function generateAnalysisJson(payload, callOpenAI, { forceRescore = false, fetchContent = null } = {}) {
  ...
  async function scoreOne(arg, camp) {
    ...
    if (!isCustom && scoresDetail && Object.keys(scoresDetail).length > 0) {
      ...
      let capPrecision = (plafondData && typeof plafondData.precision_max === 'number')
        ? plafondData.precision_max : null;
      if (capPrecision === null && diagnosticData) {
        if (isNon('mecanisme_concret_present') && isNon('seuil_present') &&
            isNon('modalite_presente') && isNon('exemple_precis_present') &&
            isNon('effet_attendu_explique')) {
          capPrecision = 14;
        }
      }

      let capReasoning = (plafondData && typeof plafondData.raisonnement_max === 'number')
        ? plafondData.raisonnement_max : null;
      if (capReasoning === null && diagnosticData) {
        if (isOui('texte_clair_mais_general') || isOui('affirmation_sans_demonstration')) {
          capReasoning = 24;
        }
      }
      ...
      const capsToApply = [
        { field: 'precision', cap: capPrecision },
        { field: 'reasoning', cap: capReasoning },
