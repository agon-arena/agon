...
- Avant d'écrire les points forts et les points faibles, vérifie chaque critère séparément et classe-le dans un état : "réussi", "partiel" ou "absent".
- Pour chaque critère, cite dans "evidence" un extrait exact de la contribution qui justifie ton diagnostic. Si aucun extrait ne prouve le critère, mets une chaîne vide.
- Ne déclare jamais absent un élément présent dans la contribution, même s'il est court, maladroit ou incomplet : écris alors "partiel" et explique ce qui manque.
- Distingue strictement : introduction absente ≠ introduction courte ; problématique absente ≠ problématique formulée sous forme de question ; définition absente ≠ définition courte ; paragraphes absents ≠ paragraphes distincts mais peu développés ; structure absente ≠ structure simple avec des connecteurs comme "D'abord", "Ensuite", "Enfin".
- Les "strengths" doivent reprendre les critères réellement remplis ou bien engagés.
- Les "weaknesses" doivent uniquement viser des critères absents ou partiels, jamais un critère réussi. Si un élément est présent mais insuffisant, formule la faiblesse comme un enrichissement nécessaire ("à développer", "à préciser"), pas comme une absence.
- Le "short_explanation" doit refléter ce diagnostic : ne transforme pas une qualité imparfaite en absence totale.
...

{
  ...
  "criteria": [
    {
      "label": "intitulé exact ou légèrement clarifié du critère",
      "score": 0,
      "max": 0,
      "status": "réussi | partiel | insuffisant | absent",
      "justification": "justification courte liée au texte évalué"
    }
  ],
  "totalScore": 0,
  "maxScore": 100,
  "improvements": [
    "axe d'amélioration directement lié à un critère partiel, insuffisant ou absent"
  ],
  "criteria_scores": {
  ...
  "criteria_diagnostics": [
    {
      "criterion": "intitulé exact du critère du barème",
      "score": 0,
      "status": "réussi | partiel | absent",
      "evidence": "extrait exact de la contribution, ou chaîne vide",
      "comment": "diagnostic court lié au critère"
    }
  ],
  "total_score": 0,
  "strengths": [
...
const CUSTOM_SCORING_DIAGNOSTIC_VERSION = 2; // barèmes personnalisés : rapport critère par critère visible
...
const MAX_UNSCORED_ARGS_PER_CAMP = 10;
...
function normalizeCustomRubricReport(p2, grid) {
  const rawCriteria = Array.isArray(p2?.criteria) && p2.criteria.length
    ? p2.criteria
    : (Array.isArray(p2?.criteria_diagnostics) ? p2.criteria_diagnostics : []);
  const criteriaScores = p2?.criteria_scores && typeof p2.criteria_scores === 'object' ? p2.criteria_scores : {};
  ...
  const criteria = rawCriteria.map((item) => {
    const label = String(item?.label || item?.criterion || item?.critere || item?.['critère'] || '').trim();
    if (!label) return null;
    const score = Math.max(0, Math.min(100, Number(item?.score ?? criteriaScores[label] ?? 0) || 0));
    const max = Math.max(0, Math.min(100, Number(item?.max ?? item?.maximum ?? parseMax(label)) || 0));
    const status = String(item?.status || '').trim() || 'partiel';
    const justification = String(item?.justification || item?.comment || item?.evidence || '').trim();
  ...
  const totalScore = Math.max(0, Math.min(100, Number(p2?.totalScore ?? p2?.total_score ?? summedScore) || 0));
  const maxScore = Math.max(1, Math.min(100, Number(p2?.maxScore ?? 100) || 100));

  return {
    summary: String(p2?.summary || p2?.short_explanation || '').trim(),
    customRubric: true,
    criteria,
    totalScore,
    maxScore,
    strengths: Array.isArray(p2?.strengths) ? p2.strengths.map(String).filter(Boolean) : [],
    improvements: Array.isArray(p2?.improvements)
      ? p2.improvements.map(String).filter(Boolean)
      : (Array.isArray(p2?.weaknesses) ? p2.weaknesses.map(String).filter(Boolean) : [])
  };
}
...
async function generateAnalysisJson(payload, callOpenAI) {
  ...
  const byRecency = (args) => [...args].sort((a, b) => {
    const tb = Date.parse(b.created_at || '') || 0;
    const ta = Date.parse(a.created_at || '') || 0;
    return tb - ta;
  });
  const previousScoredIds = (campKey) => new Set(
    (previousAnalysis?.camps?.[campKey]?.effectiveArguments || [])
      .map((arg) => String(arg?.argumentId || '').trim())
      .filter(Boolean)
  );
  const pickArgsForAnalysis = (args, campKey) => {
    const cleanArgs = filterPaste(args);
    const selected = byVotes(cleanArgs).slice(0, MAX_ARGS_PER_CAMP);
    if (!previousAnalysis) return selected;
    const selectedIds = new Set(selected.map((arg) => String(arg.id)));
    const scoredIds = previousScoredIds(campKey);
    const unscored = byRecency(cleanArgs)
      .filter((arg) => !selectedIds.has(String(arg.id)) && !scoredIds.has(String(arg.id)))
      .slice(0, MAX_UNSCORED_ARGS_PER_CAMP);
    return [...selected, ...unscored];
  };
  ...
  let argsA = pickArgsForAnalysis(rawArgsA, 'A');
  let argsB = isOpen ? [] : pickArgsForAnalysis(rawArgsB, 'B');
  ...
  async function scoreOne(arg, camp) {
    ...
    if (isCustom) {
      ...
      finalScore      = Math.max(0, Math.min(100, Number(p2.totalScore ?? p2.total_score ?? customRubricReport?.totalScore) || 0));
    ...
    return {
      ...
      category,
      criteria_diagnostics:     Array.isArray(p2.criteria_diagnostics) ? p2.criteria_diagnostics : [],
      custom_rubric_report:     customRubricReport,
      strengths:                p2.strengths || customRubricReport?.strengths || [],
      weaknesses:               p2.weaknesses || customRubricReport?.improvements || [],
