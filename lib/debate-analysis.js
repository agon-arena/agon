...
function resolveEffectiveArgs(dupResult, args) {
  ...
  for (const g of (dupResult.groups || [])) {
    ...
    const urlMembers = members.filter(m => String(m.id) !== repId && String(m.source_url || '').trim());
    urlMembers.forEach(m => kept.add(String(m.id)));
    const groupMembers = members.filter(m => !urlMembers.some(u => String(u.id) === String(m.id)));
