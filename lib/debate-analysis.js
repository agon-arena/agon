...
4. Précision / mécanisme concret — /10

L'argument donne-t-il un mécanisme, un exemple, une conséquence concrète ou une explication suffisamment précise ?

- 0–2 : idée très générale, slogan ou affirmation sans précision.
- 3–5 : début de précision, mais mécanisme encore vague.
- 6–8 : mécanisme, exemple ou conséquence assez clair.
- 9–10 : idée très concrète, précise et facilement discutable.

5. Nuance et prise en compte des limites — /5

L'argument reconnaît-il une limite, un risque ou une objection ?

- 0 : aucune nuance, caricature ou affirmation totalement unilatérale.
- 1–2 : nuance très faible ou implicite.
- 3–4 : reconnaît une limite ou une objection.
- 5 : intègre une objection importante ou une limite de manière pertinente.
...
AXE D'ANALYSE PUBLIC DÉFINI PAR LE CRÉATEUR :
{{evaluation_axis}}

Si un axe est défini ci-dessus, tu dois l'utiliser pour orienter activement la notation des contributions.
Cet axe doit influencer la note finale, en particulier les critères liés à la pertinence, à l'apport à l'arène, à la solidité et à la clarté.
Une contribution qui respecte bien cet axe doit être valorisée sur ces critères.
Une contribution qui ne respecte pas cet axe doit être pénalisée sur ces critères.

Si l'axe contient un barème chiffré clair et totalisable, ce barème remplace le barème par défaut comme base principale de calcul : applique les critères et les points du barème utilisateur, calcule le total obtenu, convertis ce total sur 100. Le barème par défaut sert alors uniquement de garde-fou minimal pour vérifier la cohérence de la note (contribution vide, hors sujet ou incohérente).
Si l'axe est une orientation qualitative, utilise-le comme priorité d'évaluation tout en conservant le barème par défaut et ses exigences générales : pertinence, clarté, cohérence, absence de hors-sujet et absence de copier-coller.

Si aucun axe n'est défini, applique simplement le barème par défaut des arènes libres.
...
- l'axe ne doit pas empêcher de distinguer une contribution faible, bonne ou excellente.
...
4. Apport à l'arène — /15

La contribution apporte-t-elle quelque chose d'utile ou d'intéressant à l'arène ?

- 0–3 : banalité, redite évidente ou contribution sans intérêt.
- 4–7 : apport faible mais présent.
- 8–11 : apport réel, ouvre une piste ou enrichit le sujet.
- 12–15 : apport notable, angle original ou point rarement soulevé.
...
Dans ce cas, la note ne doit normalement pas dépasser 14/20, sauf si l'URL pointe clairement vers une source primaire ou institutionnelle directement liée au sujet.
...
- 19–20 : URL très pertinente, source très fiable ou primaire, directement liée à l'argument, utilisée de façon précise et prudente, avec contenu réellement disponible ou lien évident et institutionnel.
...
const P2_CRITERIA = [
  ...
  { key: 'precision',  name: 'Précision / mécanisme concret',         max: 10 },
  { key: 'nuance',     name: 'Nuance et prise en compte des limites', max: 5  },
...
async function generateAnalysisJson(payload, callOpenAI) {
  ...
  async function scoreOne(arg, camp) {
    ...
    // Catégorie du score final (hors sources + sources confondues, sur 100) — c'est
    // celle-ci qui doit être affichée à côté de final_score pour rester cohérente avec
    // le nombre montré (ex. ne jamais afficher "excellent" à côté d'un score de 70/100).
    const finalCategory = toCategory(finalScore);
