...
async function scoreP2WithConsistency({ promptContent, isCustom: isCustomFlag, grid, callOpenAI, context }) {
  const [rawA, rawB] = await Promise.all([
    callOpenAI([{ role: 'user', content: promptContent }], { temperature: 0 }),
    callOpenAI([{ role: 'user', content: promptContent }], { temperature: 0 })
  ]);
  ...
  const rawC = await callOpenAI([{ role: 'user', content: promptContent }], { temperature: 0, model: 'gpt-4o' });
...
async function generateAnalysisJson(payload, callOpenAI, { forceRescore = false, fetchContent = null } = {}) {
  ...
  const argsA = byVotes(filterPaste(rawArgsAEffective));
  ...
  async function scoreOne(arg, camp) {
    ...
    const p3Raw = (hasUrl && !isCustom)
      ? await callOpenAI([{ role: 'user', content: buildP3(question, camp, arg, sourceContent) }], { temperature: 0 })
      : null;
