...
- Si l'élément attendu est présent, même formulé simplement, marque-le "réussi" et donne (presque) tous les points du critère.
- Marque "partiel" seulement si l'élément est présent mais clairement insuffisant ou incomplet.
- Marque "absent" seulement si l'élément manque vraiment, pas s'il est juste court ou maladroit.
...
async function generateAnalysisJson(payload, callOpenAI) {
  ...
  async function scoreOne(arg, camp) {
    ...
    // Le barème personnalisé (consignes nombreuses, créées librement par chaque
    // créateur d'arène) passe sur gpt-4o plutôt que gpt-4o-mini : il doit suivre
    // plus de contraintes simultanées que la grille fixe par défaut.
    const [p2Raw, p3Raw] = await Promise.all([
      callOpenAI([{ role: 'user', content: isCustom ? buildP2OpenCustom(question, content, camp, arg, grid)
        ...
        : buildP2(question, camp, arg, grid) }], { temperature: 0.1, model: isCustom ? 'gpt-4o' : undefined }),
