...
Si {{source_contents}} contient du texte réel (c'est-à-dire autre chose que "(non disponible)"), applique la VÉRIFICATION FACTUELLE suivante :
...
2. Cherche si ce fait est présent — ou clairement impliqué — dans {{source_contents}}.
...
Si {{source_contents}} vaut "(non disponible)", ne prétends jamais avoir lu le contenu. Ne pénalise pas l'absence de contenu — évalue uniquement sur le domaine et la pertinence apparente.
...
- 9–10 : Contenu disponible dans {{source_contents}} ET le fait précis avancé dans l'argument (chiffre, statistique, texte de loi, liste, affirmation centrale) est retrouvé et confirmé dans ce contenu. Source fiable ou institutionnelle. Note de 10 réservée aux cas où la source est de premier rang ET le fait central est clairement confirmé dans le contenu récupéré.
...
// le coût de gpt-4o que lorsque les deux notations mini divergent de plus de
// P2_DISAGREEMENT_THRESHOLD points, ou que l'une des deux ne parse pas. Cible le
// bug documenté dans scoreOne (mini incohérent ~1 fois sur 3) sans payer gpt-4o
// à chaque argument noté.
async function scoreP2WithConsistency({ promptContent, isCustom: isCustomFlag, grid, callOpenAI, context }) {
  const [rawA, rawB] = await Promise.all([
    callOpenAI([{ role: 'user', content: promptContent }], { temperature: 0, feature: 'debate_p2' }),
    callOpenAI([{ role: 'user', content: promptContent }], { temperature: 0, feature: 'debate_p2' })
  ]);
  ...
  const rawC = await callOpenAI([{ role: 'user', content: promptContent }], { temperature: 0, model: 'gpt-4o', feature: 'debate_arbitration' });
...
async function generateAnalysisJson(payload, callOpenAI, { forceRescore = false, fetchContent = null } = {}) {
  ...
  async function scoreOne(arg, camp) {
    ...
    const p2PromptContent = isCustom ? buildP2OpenCustom(question, content, camp, arg, grid)
      : isOpen ? buildP2Open(question, content, camp, arg, grid)
      : buildP2(question, camp, arg, grid);

    const [p2, sourceContent] = await Promise.all([
      scoreP2WithConsistency({ promptContent: p2PromptContent, isCustom, grid, callOpenAI, context: { argumentId: arg.id } }),
      (hasUrl && !isCustom && fetchContent)
        ? fetchContent(arg.source_url).catch(() => '(non disponible)')
        : Promise.resolve('(non disponible)')
    ]);

    const p3Raw = (hasUrl && !isCustom)
      ? await callOpenAI([{ role: 'user', content: buildP3(question, camp, arg, sourceContent) }], { temperature: 0, feature: 'debate_p3' })
      : null;
