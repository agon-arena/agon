...
// catalogue Mnoria n'offre pas assez de bons candidats connectés au graphe de
// l'utilisateur — jamais pour remplir artificiellement un limit=10, jamais
// sur un profil quasi vide (cf. AI_FALLBACK_MIN_ACQUISITIONS, "ne pas
// demander au LLM d'inventer une ZPD sur presque aucune donnée").
...
// readiness correcte) ET au-dessus d'un score plancher — jamais un simple
// candidat de découverte, qui ne résout pas le problème que la V2 adresse
// (relier/prolonger ce que l'utilisateur maîtrise déjà). Si au moins
// MIN_GOOD_CANDIDATES candidats passent ces deux barres, la V1 est jugée
// suffisante et aucun appel IA n'est déclenché.
const AI_FALLBACK_MIN_GOOD_CANDIDATES = 3;
const AI_FALLBACK_MIN_SCORE = 0.45;
const AI_FALLBACK_MIN_READINESS = 0.5;

// Garde-fou "profil quasi vide" (section 18, profil C) : sous ce nombre
// d'acquis, le graphe personnel est trop pauvre pour qu'une IA en tire un
// vrai "trou" pédagogique plutôt qu'une invention — la V2 reste alors
// silencieuse, V1 (mode découverte/cold start) continue seule.
const AI_FALLBACK_MIN_ACQUISITIONS = 4;
...
module.exports = {
  ...
  AI_FALLBACK_ENABLED,
  AI_FALLBACK_MIN_GOOD_CANDIDATES,
  AI_FALLBACK_MIN_SCORE,
  AI_FALLBACK_MIN_READINESS,
  AI_FALLBACK_MIN_ACQUISITIONS,
