...
// Galaxy (la "thématique" des 21 Galaxies, cf. KNOWLEDGE_GALAXY_DEFINITIONS
// côté server.js) d'un ensemble de Solar Systems (demande du 14/09/2026,
// "dans les apprentissages proposés [...] il faut associer aussi une
// thématique, comme pour les générations IA") : les candidats du moteur
// catalogue (scoredConnected/scoredDiscovery, engine.js) ne portaient jusque-
// là que solarSystemId, jamais la Galaxy elle-même — le frontend
// (learnNextThemeIconClass, qcm-du-jour.html) retombait donc sur une icône
// générique faute de theme/suggestedTheme, contrairement aux apprentissages
// déjà adoptés (themes classifiées à la génération IA, cf.
// classifyCultureGeneraleKnowledgePlacementWithAI). name volontairement omis
// ici (déjà couvert par fetchSolarSystemNames ci-dessus, requête séparée) —
// une seule colonne par appel, jamais un select() plus large que nécessaire.
async function fetchSolarSystemGalaxies(supabase, ids) {
  const uniqueIds = [...new Set((ids || []).filter((id) => Number.isInteger(id)))];
  if (!uniqueIds.length) return new Map();
  const { data, error } = await supabase.from("solar_systems").select("id, galaxy").in("id", uniqueIds);
  if (error) throw new Error(error.message);
  return new Map((data || []).map((row) => [row.id, row.galaxy]));
}
...
// absentes, encore en cours de calcul ou expirées (cooldown dépassé).
async function fetchCachedGapProposals(supabase, gapSignature, config) {
  const { data, error } = await supabase
    .from(AI_LEARNING_PROPOSALS_TABLE)
    .select("status, proposals, updated_at")
    .eq("gap_signature", gapSignature)
    .maybeSingle();
  if (error) throw new Error(error.message);
...
// Sujets libres déjà générés ("notion:custom:*") — dédupliquer une
// proposition IA contre le catalogue existant AVANT de l'afficher comme
// "nouvelle" (section 6). Prend en entrée les lignes daily_quiz déjà
// rapatriées par l'appelant (server.js, cf. getCustomTopicQuizRows). Ces
// lignes sont désormais légères : elles portent `summary:daily_quiz_question_summaries`
// et non le JSONB complet `questions`, qui avait été identifié comme le
// premier poste d'egress Supabase.
function fetchGeneratedCustomTopics(customTopicQuizRows) {
  const latestBySlot = new Map();
  for (const row of customTopicQuizRows || []) {
  ...
  for (const row of latestBySlot.values()) {
    const first = row.summary?.questions?.[0] || row.questions?.[0] || null;
    const name = first?.searchTopic || first?.sourceName || null;
    if (!name) continue;
