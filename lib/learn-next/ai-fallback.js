...
function computeGapSignature(seedTopics) {
  ...
  // durcissement "sujets autonomes".
  return crypto.createHash("sha256").update(`broad-topics-v2|${keys.join("|")}`).digest("hex").slice(0, 32);
}
...
function buildFallbackPrompt(context, config) {
  ...
  const thinProfileNote = context.seedTopics.length < 3
    ? [
        ...
        "Dans ce cas, chaque titre doit désigner un sujet encyclopédique AUTONOME et reconnu (courant de pensée, période, civilisation, discipline, grande notion ou personnalité), compréhensible sans le sujet déjà maîtrisé. Préfère par exemple « Stoïcisme », « Utilitarisme » ou « Philosophie politique » à des intitulés fabriqués comme « Éthique du bonheur et pratique quotidienne ». N'invente jamais trois micro-variantes autour du même acquis."
      ].join("\n")
    : "";
...
function sanitizeProposal(raw, context, config) {
  ...
  // Sur un profil très réduit, un titre long et composé est presque toujours
  // une micro-déclinaison artificielle du seul acquis disponible. Les sujets
  // réellement structurants restent courts et autonomes.
  if (context.seedTopics.length < 3) {
    const wordCount = title.split(/\s+/).filter(Boolean).length;
    if (wordCount > 4 || /\s+et\s+/i.test(title) || /^Éthique\s+(?:du|de la|des)\b/i.test(title)) return null;
  }
