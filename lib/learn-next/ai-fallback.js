...
function computeGapSignature(seedTopics) {
  ...
  // Versionner la politique éditoriale dans la clé : les anciennes réponses
  // trop spécialisées ne doivent pas rester servies depuis le cache après le
  // durcissement "sujets autonomes". v3 (07/09/2026, retour utilisateur "les
  // sujets proposés sont hyper précis, je veux des sujets plutôt généraux,
  // encyclopédiques") : l'exigence de généralité, jusqu'ici réservée aux
  // profils pauvres (thinProfileNote), s'applique désormais à TOUS les
  // profils — invalide le cache des propositions générées sous l'ancienne
  // politique, plus large/fabriquée.
  return crypto.createHash("sha256").update(`broad-topics-v3|${keys.join("|")}`).digest("hex").slice(0, 32);
}
...
function buildFallbackPrompt(context, config) {
  ...
  // L'exigence de généralité du TITRE elle-même (sujet encyclopédique
  // autonome) n'est plus limitée à ce cas — cf. la consigne universelle plus
  // bas (section "title") depuis le 07/09/2026.
  ...
  return [
    ...
    "- jamais un intitulé FABRIQUÉ qui décrit précisément le lien avec un acquis (ex. « Éthique du bonheur et pratique quotidienne », « Influence de la Guerre de Cent Ans sur l'artillerie moderne », « Liens entre stoïcisme et gestion des émotions ») — même pour un sujet de type « bridge »/« continuity », le TITRE doit toujours désigner un sujet ENCYCLOPÉDIQUE autonome et reconnu (courant de pensée, période historique, civilisation, discipline, grande notion, événement majeur ou personnalité), qui existerait tel quel dans une encyclopédie indépendamment de cet utilisateur. Bons exemples : « Stoïcisme », « Guerre de Cent Ans », « Thermodynamique », « Empire ottoman ». Le lien avec ses acquis se justifie UNIQUEMENT dans `reason`, jamais dans le titre.",
    ...
    "- title : le sujet encyclopédique autonome lui-même (2 à 4 mots dans la grande majorité des cas), jamais une question, jamais une phrase complète, jamais un titre composé avec « et »/« dans »/« à travers »/« influence de »/« liens entre » ;",
...
function sanitizeProposal(raw, context, config) {
  ...
  // Garde-fou déterministe (07/09/2026, retour utilisateur "les sujets
  // proposés sont hyper précis, je veux des sujets plutôt généraux,
  // encyclopédiques") : appliqué à TOUS les profils désormais (plus
  // seulement les profils pauvres) — un titre long/composé reste presque
  // toujours un intitulé fabriqué décrivant un lien précis avec un acquis,
  // jamais un vrai sujet encyclopédique autonome (ceux-ci restent courts :
  // "Stoïcisme", "Guerre de Cent Ans", "Empire ottoman"...). Seuil à 6 mots
  // (plutôt que 4) pour ne pas rejeter à tort de vrais sujets un peu plus
  // longs ("Chute de l'Empire romain d'Occident") : ne cible que les
  // constructions manifestement fabriquées (liaison "et"/"dans"/"à travers",
  // ou un tour explicite "influence de"/"liens entre").
  const wordCount = title.split(/\s+/).filter(Boolean).length;
  if (
    wordCount > 6 ||
    /\s+et\s+/i.test(title) ||
    /\s+(dans|à travers|au sein de)\s+/i.test(title) ||
    /^(influence|impact)\s+(de|du|des)\b/i.test(title) ||
    /^liens?\s+entre\b/i.test(title) ||
    /^Éthique\s+(?:du|de la|des)\b/i.test(title)
  ) return null;
