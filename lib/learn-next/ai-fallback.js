...
// ── Déclenchement (sections 1 et 18 de la mission) ──────────────────────────
// "V1 suffisante" = au moins MIN_GOOD_CANDIDATES candidats déjà bons (calculé
// dans engine.js, jamais recalculé ici). "Profil quasi vide" = trop peu
// d'acquis pour qu'un LLM en tire un vrai trou pédagogique plutôt qu'une
// invention (profil C de la mission : prudence, pas de ZPD fabriquée à partir
// de presque rien).
function shouldTriggerFallback({ qualitySignal, coldStart }, config) {
  if (!config.AI_FALLBACK_ENABLED) return false;
  if (coldStart) return false;
  if (!qualitySignal || qualitySignal.sufficient) return false;
  if ((qualitySignal.totalAcquisitions || 0) < config.AI_FALLBACK_MIN_ACQUISITIONS) return false;
  return true;
}
...
function buildFallbackPrompt(context, config) {
  ...
  return [
    ...
    "- jamais un sujet trop général (ex. \"L'histoire de France\") ni à l'inverse un détail extrêmement spécialisé sans lien clair avec ses connaissances ;",
    ...
    "- title : titre court et précis du sujet (jamais une question, jamais une phrase complète) ;",
