...
// Dédoublonnage INTRA-POOL (revue du 30/08/2026, validation en conditions
// réelles) : deux candidats DISTINCTS (subject_source_id différents) peuvent
// porter un nom identique ou quasi-identique — ex. plusieurs fiches "Capitales
// du monde" générées séparément par différents visiteurs sans jamais être
// fusionnées, confirmé en base de production. Le filtre de redondance
// existant (isRedundant, ci-dessus) ne compare un candidat qu'aux ACQUIS de
// l'utilisateur, jamais aux AUTRES candidats du même pool — sans ce passage,
// deux fiches quasi-identiques peuvent occuper 2 des 3 préconisations
// finales. Trié par finalScore décroissant avant élagage : au sein d'un
// groupe équivalent, seul le mieux scoré survit — jamais un choix arbitraire,
// jamais une pénalité de score. N'affecte NI le scoring NI passesRelevanceGate
// (déjà appliqués en amont) : purement un filtre du pool déjà éligible.
function dedupeCandidatesByName(candidates) {
  const sorted = [...candidates].sort((a, b) => b.finalScore - a.finalScore);
  const kept = [];
  for (const candidate of sorted) {
    const isDuplicate = candidate.name && kept.some((k) => k.name && isSafeTopicEquivalent(candidate.name, k.name));
...
async function computeLearnNextRecommendations(deps, { userId, limit, debug = false, now = new Date() }) {
  ...
  const [nodes, discoveryRows] = await Promise.all([
    ...
    repository.fetchDiscoveryCandidates(supabase, {
      ...
      // Cold-start (revue du 30/08/2026) : trie par acquisition_count plutôt
      // que par importance_score dès le fetch, pour ne jamais manquer un
      // sujet réellement populaire mais peu connecté au graphe (cf.
      // repository.js pour le constat en base de production).
      orderBy: coldStart ? "acquisition_count" : "importance_score"
    })
  ]);
  ...
  for (const candidate of connectedCandidatesMap.values()) {
    ...
    // "Bon candidat" (revue du 30/08/2026) : appliqué ici directement sur ce
    // qui sera montré (filtré avant assembleRecommendations plus bas), pas
    // seulement compté à part — cf. scoring.passesRelevanceGate.
    const isGoodCandidate = scoring.passesRelevanceGate({ type, finalScore, readiness }, config);
  ...
  for (const row of discoveryRows) {
    ...
    // Cold-start (revue du 30/08/2026) : classement PAR POPULARITÉ RÉELLE
    // (acquisition_count, cf. scoring.computePopularityScore) plutôt que par
    // le score composite normal — à 0 acquisition, zpd/connections/bridge
    // sont structurellement nuls et interestScore/discoveryScore identiques
    // pour tous les candidats (aucun signal personnel pour les différencier),
    // donc computeFinalScore classerait de toute façon uniquement par
    // importance_score — qui mélange à parts égales popularité ET
    // connectivité du graphe (cf. computeImportanceScore), jamais un vrai
    // "les plus adoptés d'abord". Hors cold-start, comportement inchangé.
    const acquisitionCount = Number.isFinite(row.acquisition_count) ? row.acquisition_count : 0;
    const finalScore = coldStart
      ? scoring.computePopularityScore({ acquisitionCount, importanceScore }, config)
      : scoring.computeFinalScore(
    ...
    // En cold-start, le pool "discovery" (désormais trié par popularité
    // réelle, cf. ci-dessus) est la SEULE source disponible — ce sont par
    // construction de bons sujets d'entrée, jamais une fausse personnalisation
    // (aucun voisin acquis n'existe encore pour en juger autrement). Hors
    // cold-start, "discovery" ne peut de toute façon jamais franchir
    // AI_FALLBACK_MIN_SCORE (cf. scoring.passesRelevanceGate) : finalScore
    // reste toujours faux ici.
    const isGoodCandidate = coldStart || scoring.passesRelevanceGate({ type: "discovery", finalScore, readiness: 0 }, config);
    ...
    scoredDiscovery.push({
      ...
      isGoodCandidate,
      reason,
      debugScores: debug ? { importanceScore, discoveryScore, interestScore, novelty, saturationPenalty, acquisitionCount } : null
    });
  }

  // Priorité catalogue stricte (revue du 30/08/2026) : seuls les candidats
  // ayant franchi un vrai seuil de pertinence (isGoodCandidate) alimentent
  // l'assemblage final — jamais de remplissage à `safeLimit` avec un candidat
  // techniquement disponible mais insuffisamment pertinent. S'il en manque,
  // c'est à l'appelant (server.js) de solliciter l'IA pour les places restantes.
  const eligibleCandidates = dedupeCandidatesByName(
    [...scoredConnected, ...scoredDiscovery].filter((c) => c.isGoodCandidate)
  );
  const picked = scoring.assembleRecommendations(eligibleCandidates, {
    limit: safeLimit,
    mixRatios: config.MIX_RATIOS,
    maxPerSolar: config.MAX_PER_SOLAR_IN_RESPONSE
  });

  // Galaxy ("thématique", cf. KNOWLEDGE_GALAXY_DEFINITIONS côté server.js) de
  // chaque recommandation retenue (demande du 14/09/2026, "il faut associer
  // aussi une thématique, comme pour les générations IA") : jusque-là,
  // recommendations ne portait que solarSystemId (un id opaque), jamais la
  // Galaxy elle-même — le frontend (learnNextThemeIconClass) retombait donc
  // sur une icône générique pour ces candidats catalogue, contrairement aux
  // connaissances déjà adoptées (themes classifiées à la génération IA).
  // Requête groupée sur les seuls ids réellement retenus, jamais tout le pool
  // de candidats écartés en amont.
  const galaxyBySolarId = await repository.fetchSolarSystemGalaxies(
    supabase,
    picked.map((item) => item.solarSystemId).filter(Boolean)
  );
  ...
  const recommendations = picked.map((item) => ({
    ...
    theme: item.solarSystemId ? (galaxyBySolarId.get(item.solarSystemId) || null) : null,
  ...
  // Contexte exposé pour la V2 (fallback IA) — additif, ne change rien au
  // contrat existant (`recommendations`/`coldStart`) consommé par V1 et ses
  // tests. Le nombre de "bons candidats" n'a plus besoin d'être compté à part
  // (revue du 30/08/2026) : `recommendations.length` EST déjà ce nombre,
  // puisque `picked` provient exclusivement d'`eligibleCandidates` ci-dessus —
  // c'est à l'appelant (server.js) de comparer cette longueur à la cible pour
  // savoir combien de places l'IA doit combler. topCandidateNames sert à dire
  // au LLM "ne propose pas l'équivalent de ceci" (section 3), quel que soit
  // leur type — déjà limité aux seuls items réellement montrés à l'utilisateur.
  const qualitySignal = {
    totalAcquisitions,
    topCandidateNames: picked.map((item) => item.name).filter(Boolean).slice(0, config.AI_FALLBACK_EXISTING_CANDIDATES_LIMIT)
  };

  return { recommendations, coldStart, qualitySignal };
}
