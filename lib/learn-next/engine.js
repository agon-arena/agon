...
async function computeLearnNextRecommendations(deps, { userId, limit, debug = false, now = new Date() }) {
  ...
  // Signal de suffisance pour la V2 (fallback IA, mission du 27/08/2026) :
  // compté ici, au fil du scoring déjà en cours, plutôt que recalculé à part
  // — "bon candidat" = connecté au graphe (bridge, ou continuity avec une
  // readiness correcte) ET au-dessus d'un score plancher. Les candidats de
  // découverte ne comptent JAMAIS : ils ne répondent pas à la question que
  // la V2 adresse (relier/prolonger ce que l'utilisateur maîtrise déjà).
  let goodCandidateCount = 0;
  ...
  for (const candidate of connectedCandidatesMap.values()) {
    ...
    if (
      finalScore >= config.AI_FALLBACK_MIN_SCORE &&
      (type === "bridge" || (type === "continuity" && readiness >= config.AI_FALLBACK_MIN_READINESS))
    ) {
  ...
  // Signal de suffisance exposé pour la V2 (fallback IA) — additif, ne change
  // rien au contrat existant (`recommendations`/`coldStart`) consommé par V1
  // et ses tests. topCandidateNames sert à dire au LLM "ne propose pas
  // l'équivalent de ceci" (section 3), quel que soit leur type.
  const qualitySignal = {
    goodCandidateCount,
    sufficient: goodCandidateCount >= config.AI_FALLBACK_MIN_GOOD_CANDIDATES,
    totalAcquisitions,
