...
function buildPdfBlocks(pages, maxChars = PDF_BLOCK_MAX_CHARS) {
  ...
  for (const page of (pages || [])) {
    ...
    for (let offset = 0; offset < text.length; offset += Math.max(1, maxChars - 32)) {
      const fragment = text.slice(offset, offset + Math.max(1, maxChars - 32));
...
function deduplicatePdfKnowledge(items, limit) {
  ...
  for (const item of (Array.isArray(items) ? items : [])) {
    const knowledge = String(item?.knowledge || "").trim().replace(/\s+/g, " ").slice(0, 500);
