...
async function applyBatchResults({ supabase, batchId, resultsByCustomId }) {
  const { data: rows, error } = await supabase
    .from(CALLS_TABLE)
    .select("id, queue_id, call_key, custom_id, request_payload")
    .eq("batch_id", batchId)
    .eq("status", "batch_submitted");
  if (error) throw new Error(error.message);
  for (const row of rows || []) {
    ...
    // précisément le coût des QCM en batch et non-batch") : chaque ligne de
    // résultat Batch porte déjà usage.{prompt_tokens,completion_tokens} —
    // enregistrée ici dans ai_usage_log EXACTEMENT comme le chemin synchrone
    // (même table, même fonction recordAiUsage), avec isBatch:true pour que
    // la remise Batch officielle (-50%) soit appliquée automatiquement.
    // Fire-and-forget (jamais awaité), comme toute la télémétrie du projet —
    // une panne de log ne doit jamais bloquer la progression d'un sujet.
    if (result.ok) {
      const { inputTokens, outputTokens, cachedTokens } = extractUsage(result.usage);
      recordAiUsage(supabase, {
        feature: row.call_key, model: row.request_payload?.model,
        inputTokens, outputTokens, cachedTokens,
        generationId: row.queue_id != null ? String(row.queue_id) : null,
...
// Cache pour les étapes SYNCHRONES rejouables (recherche Brave, cf.
// server.js braveSearchRaw) — jamais un Batch, contrairement à resolveStep :
// le résultat est calculé immédiatement dès la première rencontre puis
// simplement relu aux cycles suivants, sans jamais passer par un statut
// "pending"/"batch_submitted". Corrige l'incident quota Brave du 08/09/2026 :
// le pipeline rejoué à chaque cycle scheduler refaisait entièrement la
// recherche Brave tant qu'un sujet attendait un résultat Batch OpenAI plus
// loin dans le pipeline.
async function resolveCachedSyncStep({ supabase, queueId, callKey, occurrence, requestPayload, compute }) {
  const customId = buildCustomId(queueId, callKey, occurrence);
  const { data: existing, error: selectError } = await supabase
    .from(CALLS_TABLE)
    ...
    .maybeSingle();
  if (selectError) throw new Error(selectError.message);
  if (existing) return JSON.parse(existing.result_content);

  const result = await compute();
  const { error: upsertError } = await supabase
    .from(CALLS_TABLE)
    .upsert(
      { queue_id: queueId, call_key: callKey, occurrence, custom_id: customId, status: "completed", request_payload: requestPayload, result_content: JSON.stringify(result) },
      { onConflict: "queue_id,call_key,occurrence" }
    );
