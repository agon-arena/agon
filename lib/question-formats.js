...
function attachPedagogicalRanks(questions, rankedKnowledge) {
  const rankByFact = new Map((Array.isArray(rankedKnowledge) ? rankedKnowledge : []).map((k) => [normalizeFactText(k.fact), k.pedagogicalRank]));
  return (Array.isArray(questions) ? questions : []).map((question) => {
    const rank = rankByFact.get(normalizeFactText(question?.knowledgeTarget));
    return Number.isInteger(rank) ? { ...question, pedagogicalRank: rank } : question;
  });
}
...
function isMasterEligibleQuiz(questions) {
  return Array.isArray(questions)
    && questions.length >= MIN_MASTER_QUESTIONS
    && questions.every((question) => Number.isInteger(question?.pedagogicalRank));
}
