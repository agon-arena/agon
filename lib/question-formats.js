...
// réel "La photosynthèse", progressive_status="ready" avec 14 questions
// cumulées) : exiger encore MIN_MASTER_QUESTIONS=15 pour un master
// progressif déjà "ready" contredit le sens même de ce statut — chaque
// niveau (elementary_ready, deepening_ready) a déjà sa propre validation de
// suffisance AVANT de faire progresser progressive_status jusqu'à "ready"
// (cf. ensureProgressiveElementaryGenerated/continueProgressiveGeneration) ;
// ce seuil de 15 est un vestige du master legacy (avant les QCM progressifs)
// et ne devrait jamais s'appliquer une fois "ready" atteint. Confirmé pas
// spécifique au Batch : un master "ready" généré en synchrone AVANT ce
// chantier (11 questions cumulées) présentait exactement le même défaut de
// réutilisation. Un master progressif "ready" est donc désormais TOUJOURS
// éligible dès qu'il a au moins une question valide (déjà garanti non vide
// et pedagogicalRank entier par l'appelant ci-dessous) — jamais un second
// seuil de quantité par-dessus une validation par niveau déjà faite.
function progressiveEligibilityMinimum(progressiveStatus, curriculum) {
  if (progressiveStatus === "ready") return 1;
  const list = Array.isArray(curriculum) ? curriculum : [];
