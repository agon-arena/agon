// Génération IA du libellé court (cloud_label) destiné aux futures bulles Mnoria.
...
// Coupe au dernier espace pour ne jamais trancher un mot en plein milieu (sauf si aucun espace
// assez tôt dans la limite, auquel cas coupe brute) — réutilisé par sanitizeCloudLabel ci-dessous
// et par resolveOrCreateSolarSystem (server.js) pour les noms de système solaire, qui partagent
// la même contrainte : tenir dans une bulle Mnoria (même moteur d'affichage, cf. tagTrendCloud.js).
function truncateToBubbleLabel(value, maxLength = MAX_LABEL_LENGTH) {
...
function sanitizeCloudLabel(raw) {
  ...
  label = truncateToBubbleLabel(label);
...
async function generateCloudLabel(fields, supabase = null) {
  ...
  try {
    ...
    if (!r.ok) {
      recordAiUsage(supabase, { feature: "debate_cloud_label", model: "gpt-4o-mini", latencyMs: Date.now() - requestStartedAt, success: false, error: `openai http ${r.status}` });
      return null;
    }
    const data = await r.json();
    const { inputTokens, outputTokens, cachedTokens } = extractUsage(data?.usage);
    recordAiUsage(supabase, { feature: "debate_cloud_label", model: "gpt-4o-mini", inputTokens, outputTokens, cachedTokens, latencyMs: Date.now() - requestStartedAt, success: true });
...
module.exports = { generateCloudLabel, sanitizeCloudLabel, truncateToBubbleLabel, MAX_LABEL_LENGTH };
