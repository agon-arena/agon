...
function buildCurriculumPrompt(subject, contextHint, groundingText = null) {
  const lines = [
...
function parseCurriculumItems(raw) {
  ...
  for (const entry of raw) {
    ...
    items.push({ id, knowledgeTarget, order });
  }
...
function normalizeCurriculumOrder(items) {
  ...
  return sorted.map((item, index) => ({ id: `k${index + 1}`, knowledgeTarget: item.knowledgeTarget, order: index + 1 }));
}
...
function buildCurriculumRepairPrompt(subject, contextHint, neededCount, existingKnowledge, groundingText = null) {
