...
// généralisé) : le "level" d'une connaissance n'est JAMAIS décidé
// DIRECTEMENT par le modèle — toujours dérivé, côté code, de sa position
// (`order`) une fois le curriculum final normalisé et de la taille totale N.
// `computeCurriculumSplit(N)` calcule cette proportion 25 % elementary /
// 25 % deepening / 50 % expert (avec un plancher de 4 par petit niveau) ;
// `levelForOrder` applique cette proportion à une position donnée.
//
// Correction E2 (diagnostic qualité pédagogique du 13/09/2026, cas réel
// "Charlemagne" — la position brute dans la liste du modèle ne reflète pas
// fiablement l'importance pédagogique réelle : des mécanismes précis
// ("l'armée peut être convoquée chaque année au printemps et en été")
// atterrissaient en Élémentaire tandis que des fondamentaux structurants
// n'étaient tout simplement jamais choisis) : `order` reste la SEULE source
// de vérité du niveau (rien ne change dans `levelForOrder`/
// `computeCurriculumSplit` eux-mêmes), mais la position qu'occupe chaque
// connaissance dans la liste normalisée n'est plus purement l'ordre BRUT
// renvoyé par le modèle — `normalizeCurriculumOrder` trie désormais D'ABORD
// par `importance` déclarée ("high" avant "medium" avant "low", même
// nomenclature que lib/knowledge-admission.js + lib/question-formats.js
// IMPORTANCE_RANK_ORDER/rankAdmittedKnowledge), l'ordre du modèle ne servant
// plus qu'à départager deux connaissances de MÊME importance. Le modèle
// choisit donc un signal explicite d'importance ; le code reste seul maître
// de la répartition finale par niveau — jamais un second système parallèle.
...
// IMPORTANCE_RANK (correction E2, 13/09/2026) : même nomenclature et même
// ordre que IMPORTANCE_RANK_ORDER (lib/question-formats.js, pipeline
// d'admission legacy) — "low" existe ici en plus (absent là-bas, où une
// connaissance "low" n'est jamais admise du tout) parce qu'une connaissance
// de faible importance pédagogique reste légitime ICI, tant qu'elle atterrit
// en Expert plutôt qu'en Élémentaire : c'est précisément ce que ce signal
// sert à garantir, jamais à exclure une connaissance par ailleurs valable.
...
function buildCurriculumPrompt(subject, contextHint, groundingText = null, identifiedSourcesBlock = null) {
  ...
  lines.push(
    ...
    "TEST D'ADMISSION AVANT CHAQUE knowledgeTarget : retiens une information seulement si elle répond OUI à ces trois questions : (1) est-elle centrale ou vraiment éclairante pour comprendre CE sujet précis, et pas seulement liée au contexte ? (2) peut-elle être mémorisée comme une connaissance autonome, spécifique et durable ? (3) permettra-t-elle de créer ensuite une question utile, dont la réponse exige réellement cette connaissance plutôt qu'un simple bon sens ou une devinette ? Si une information échoue à l'un de ces trois tests, écarte-la même si elle est vraie.",
    "Écarte donc les connaissances trop périphériques ou trop génériques : titre, source, auteur, date de publication, détail d'actualité sans portée durable, anecdote isolée, information applicable à presque n'importe quel sujet, simple reformulation du libellé du sujet, ou contexte général qui n'apprend rien de substantiel sur le sujet lui-même. Une bonne connaissance doit changer quelque chose à la compréhension du sujet, pas seulement meubler une fiche.",
    ...
    "",
    // Concept avant attribut (diagnostic qualité éditoriale du 12/09/2026,
    // cas réel "Révolution française" — le curriculum retenait plusieurs
    // connaissances décrivant les positions, les membres et l'opposition aux
    // Montagnards d'un courant révolutionnaire (attachement aux libertés
    // individuelles, noms de plusieurs dirigeants...) SANS jamais retenir
    // explicitement le concept auquel elles se rattachent toutes : les
    // Girondins. Un lecteur mémorisant ces attributs sans jamais rencontrer
    // le nom du groupe ne peut ni les relier entre eux, ni reconnaître le
    // concept ailleurs.) Règle générale, jamais spécifique à un sujet : la
    // COUVERTURE ÉQUILIBRÉE ci-dessus vise la répartition ENTRE sous-thèmes,
    // celle-ci vise la structure INTERNE d'un même sous-thème.
    "CONCEPT AVANT ATTRIBUT : si plusieurs des connaissances que tu retiens décrivent en réalité des propriétés, des membres, des positions ou des actions d'un même concept, groupe, courant, institution ou catégorie nommé(e) dans les sources, alors CE concept lui-même doit être retenu comme connaissance à part entière — jamais seulement ses attributs épars. Mauvais exemple : retenir séparément \"attaché aux libertés individuelles\", \"opposé aux mesures d'exception\" et les noms de plusieurs dirigeants d'un même courant sans jamais nommer explicitement ce courant. Bon exemple : nommer d'abord le concept (ex. le nom du courant ou du groupe), puis, si la place le permet, un ou deux attributs réellement distinctifs. Ne t'interdis pas pour autant de retenir un attribut isolé et notable quand aucun concept nommé ne le chapeaute dans les sources — cette règle vise seulement le cas où plusieurs attributs d'UN MÊME concept nommé s'accumulent sans jamais que ce concept soit lui-même une entrée du curriculum.",
    "",
  ...
  lines.push(
    ...
    "",
    // IMPORTANCE PÉDAGOGIQUE (correction E2, diagnostic qualité pédagogique
    // du 13/09/2026, cas réel "Charlemagne" — l'ORDRE de la liste ci-dessus
    // reste demandé, mais un ordre seul ne suffit pas à garantir qu'une
    // connaissance vraiment fondamentale atterrisse en Élémentaire : ce
    // signal explicite, distinct de l'ordre, est celui qui pèsera le plus
    // dans la répartition finale par niveau, calculée automatiquement côté
    // code. Même nomenclature ("high"/"medium"/"low") que le pipeline
    // d'admission de connaissances existant, pour rester cohérent avec le
    // reste du système.
    "IMPORTANCE PÉDAGOGIQUE : qualifie EN PLUS chaque connaissance retenue avec un champ \"importance\", qui pèsera fortement dans la répartition finale entre Élémentaire/Avancé/Expert (calculée automatiquement, jamais choisie par toi) :",
    "- \"high\" = FONDAMENTALE : quelqu'un ayant étudié le sujet devrait difficilement pouvoir l'ignorer. Identité ou définition centrale, événement majeur, mécanisme indispensable, cause ou conséquence essentielle, découverte/œuvre/institution/notion structurante.",
    "- \"medium\" = IMPORTANTE mais non indispensable au premier socle : mécanisme secondaire, contexte, nuance importante, approfondissement, conséquence ou exemple significatif — nécessaire pour vraiment COMPRENDRE le sujet, pas pour le simple socle initial.",
    "- \"low\" = détail spécialisé, anecdote, exception, controverse secondaire ou précision enrichissante — a toute sa place dans le curriculum, mais ne doit jamais prendre la place d'un \"high\" absent.",
    "ATTENTION : rare, précis ou original ne veut JAMAIS dire important — ne valorise jamais artificiellement une connaissance \"high\" ou \"medium\" simplement parce qu'elle est surprenante, peu connue ou inhabituelle. À l'inverse, une connaissance largement connue ou en apparence \"évidente\" (l'identité du sujet, sa date ou son événement le plus marquant...) reste \"high\" si elle est réellement fondamentale — ne la déclasse jamais en \"medium\"/\"low\" au seul prétexte qu'elle semble trop simple ou trop connue.",
    "Sois réaliste et sélectif sur \"high\" : marquer presque tout \"high\" reviendrait à ne plus rien distinguer — réserve ce niveau aux connaissances qu'une personne cultivée sur ce sujet ne devrait vraiment pas ignorer."
  );
  if (identifiedSourcesBlock) {
    lines.push(
      "",
      ...
      "",
      `Réponds uniquement en JSON strict : si le sujet est ambigu (cf. plus haut), uniquement sous la forme ${TOPIC_VALIDATION_AMBIGUOUS_JSON_EXAMPLE} ; sinon sous la forme {"topicValidation":{"status":"valid","normalizedTopic":"..."|null},"curriculum":[{"id":"k1","knowledgeTarget":"phrase factuelle courte et autonome","order":1,"importance":"high|medium|low","source_id":"SOURCE_1","evidence_text":"extrait exact copié depuis SOURCE_1"}, ...]} — un objet par connaissance choisie (entre 15 et 20 au total), id de "k1" à "kN", order de 1 à N dans le même ordre pédagogique croissant. CHAQUE objet, sans exception, doit porter "importance", "source_id" ET "evidence_text".`
    );
  } else {
    lines.push(
      "",
      `Réponds uniquement en JSON strict : si le sujet est ambigu (cf. plus haut), uniquement sous la forme ${TOPIC_VALIDATION_AMBIGUOUS_JSON_EXAMPLE} ; sinon sous la forme {"topicValidation":{"status":"valid","normalizedTopic":"..."|null},"curriculum":[{"id":"k1","knowledgeTarget":"phrase factuelle courte et autonome","order":1,"importance":"high|medium|low"}, ...]} — un objet par connaissance choisie (entre 15 et 20 au total), id de "k1" à "kN", order de 1 à N dans le même ordre pédagogique croissant. CHAQUE objet doit porter "importance".`
    );
  }
  return lines.filter((line) => line !== null).join("\n");
}

// Normalise le champ "importance" renvoyé par le modèle (correction E2) —
// conservateur par construction, même philosophie que le reste de ce
// parsing : une valeur absente, mal orthographiée ou hors énumération
// retombe sur DEFAULT_IMPORTANCE ("medium") plutôt que de rejeter l'item
// entier ou de planter — une connaissance par ailleurs valable ne doit
// jamais disparaître pour un simple champ d'importance malformé.
function normalizeImportance(raw) {
  const value = typeof raw === "string" ? raw.trim().toLowerCase() : "";
  return Object.prototype.hasOwnProperty.call(IMPORTANCE_RANK, value) ? value : DEFAULT_IMPORTANCE;
}
...
// connue.
// "importance" (correction E2, 13/09/2026) : contrairement à source_id/
// evidence_text ci-dessous (capturés seulement par paire), TOUJOURS présent
// sur l'item parsé (jamais undefined) — normalizeImportance retombe sur
// "medium" plutôt que d'omettre le champ, pour que normalizeCurriculumOrder
// (qui trie dessus) ait toujours une valeur exploitable, y compris pour un
// appelant/test qui construirait des items sans jamais fournir ce champ.
...
// aucune valeur). Rétrocompatible au caractère près pour CES deux champs
// seulement : un item sans eux (TOUS les appelants avant ce paramètre, et
// tout appel sans identifiedSourcesBlock) ne les porte toujours pas.
function parseCurriculumItems(raw) {
  ...
  for (const entry of raw) {
    ...
    const item = { id, knowledgeTarget, order, importance: normalizeImportance(entry?.importance) };
...
// Rang d'importance d'un item, avec repli — un item sans champ "importance"
// (tout appelant/test antérieur à la correction E2, qui n'a jamais fourni ce
// champ) retombe sur le rang de DEFAULT_IMPORTANCE ("medium"), jamais une
// exception ni un tri instable.
function importanceRankOf(item) {
  return IMPORTANCE_RANK[item?.importance] ?? IMPORTANCE_RANK[DEFAULT_IMPORTANCE];
}

// Renormalise le curriculum ADMIS (déjà filtré aux connaissances acceptées —
// y compris les ajouts de réparation, placés après les items d'origine, cf.
// server.js) : comble tout "trou" laissé par un rejet en réassignant
// id=k1..kN et order=1..N de façon strictement séquentielle. Pure, sans
// effet de bord — ne calcule PAS encore le niveau (cf. assignCurriculumLevels,
// appelée seulement après : le niveau dépend de la taille finale N, connue
// uniquement une fois cette normalisation faite).
//
// Tri PAR IMPORTANCE D'ABORD, PUIS PAR ORDRE (correction E2, diagnostic
// qualité pédagogique du 13/09/2026 — cf. commentaire de tête du fichier) :
// la position qu'occupe un item dans le curriculum NORMALISÉ (qui détermine
// ensuite son niveau via levelForOrder/computeCurriculumSplit, INCHANGÉS)
// dépend désormais D'ABORD de son "importance" déclarée ("high" avant
// "medium" avant "low"), l'ordre pédagogique du modèle ne départageant plus
// que deux connaissances de MÊME importance. Comportement STRICTEMENT
// IDENTIQUE à avant cette correction pour tout appelant qui ne fournit
// jamais ce champ (importanceRankOf retombe alors sur le même rang par
// défaut pour tous les items, donc sur un tri par ordre seul, inchangé au
// caractère près) — y compris le cas dégénéré où le modèle marquerait TOUT
// "high" (même rang partout -> repli sur l'ordre seul, jamais une
// répartition cassée).
//
// Spread de l'item AVANT id/order (V1 evidence grounding, 03/09/2026) :
// préserve source_id/evidence_text/importance quand présents, jamais une
// reconstruction champ par champ qui les aurait silencieusement perdus en
// route. Aucun changement pour un item sans ces champs (comportement
// historique) : le spread d'un objet {id, knowledgeTarget, order} suivi de
// {id, order} écrasés produit exactement le même résultat qu'avant, au
// caractère près.
function normalizeCurriculumOrder(items) {
  const sorted = [...(Array.isArray(items) ? items : [])].sort((a, b) => {
    const importanceDiff = importanceRankOf(a) - importanceRankOf(b);
    return importanceDiff !== 0 ? importanceDiff : a.order - b.order;
  });
...
function buildCurriculumRepairPrompt(subject, contextHint, neededCount, existingKnowledge, groundingText = null, identifiedSourcesBlock = null) {
  ...
  lines.push(
    ...
    "Applique aussi le même TEST D'ADMISSION que pour le curriculum initial : chaque ajout doit être central ou vraiment éclairant pour CE sujet précis, mémorisable comme connaissance autonome, et capable de produire ensuite une question utile. N'ajoute jamais un titre, une source, une date de publication, un contexte vague, une anecdote isolée ou une information simplement liée au sujet mais peu instructive.",
