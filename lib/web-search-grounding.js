...
// l'article. La cause principale corrigée ici est la STRATÉGIE de sélection
// (cf. lib/source-excerpt-selection.js, passages représentatifs répartis
// sur le document plutôt que les premiers caractères) — ce budget n'est
// relevé que modérément (+60 %, jamais ×4) : préférer un budget raisonnable
// intelligemment choisi à un budget massif pris naïvement, conformément à
// la demande explicite ("je préfère 4000 caractères intelligemment
// sélectionnés à 10000 caractères pris naïvement en tête"). Impact tokens
// mesuré négligeable (cf. rapport final, section coût).
const WEB_SEARCH_EXCERPT_MAX_CHARS = 4000;
...
// Encyclopédies de référence à privilégier quand c'est possible (demande du
// 31/08/2026, étendue le 09/09/2026 à Britannica/Universalis) : un simple tri
// stable place ces résultats en tête de la liste présentée à l'IA de
// sélection, dans l'ordre Wikipédia > Britannica > Universalis — ne change
// jamais QUI est éligible (le filtre déterministe ci-dessous reste inchangé)
// ni ne contourne son jugement de pertinence/fiabilité (cf.
// buildSourceSelectionPrompt, qui porte la même consigne de priorité), juste
// l'ordre de présentation.
const WIKIPEDIA_DOMAIN_PATTERN = /(^|\.)wikipedia\.org$/;
const BRITANNICA_DOMAIN_PATTERN = /(^|\.)britannica\.com$/;
const UNIVERSALIS_DOMAIN_PATTERN = /(^|\.)universalis\.fr$/;

function referenceDomainRank(domain) {
  if (WIKIPEDIA_DOMAIN_PATTERN.test(domain)) return 0;
  if (BRITANNICA_DOMAIN_PATTERN.test(domain)) return 1;
  if (UNIVERSALIS_DOMAIN_PATTERN.test(domain)) return 2;
  return 3;
}
...
function buildSourceSelectionPrompt(subject, contextHint, candidates, maxSelected = WEB_SEARCH_MAX_SELECTED_SOURCES) {
  ...
  return [
    ...
    // 11 — la préférence Wikipédia ci-dessous reste utile mais ne doit
    // jamais aboutir à 3 sources quasi redondantes, toutes de type
    // encyclopédie/infobox généraliste, ce qui pousse mécaniquement la fiche
    // vers une accumulation de faits descriptifs plutôt qu'une explication).
    // Prompt uniquement, aucune requête Brave supplémentaire : agit sur le
    // même lot de candidats déjà récupéré.
    `Quand plusieurs sources valables sont disponibles, préfère une COMBINAISON complémentaire plutôt que ${maxSelected} sources qui se contentent de répéter le même type de contenu descriptif (plusieurs encyclopédies généralistes redondantes, plusieurs pages de type infobox) : une source encyclopédique générale (typiquement Wikipédia, cf. priorité ci-dessous) PLUS, quand elles existent et sont pertinentes, une ou deux sources institutionnelles, académiques ou spécialisées qui apportent un angle différent (mécanisme, contexte, analyse) plutôt que les mêmes données déjà couvertes. N'écarte cependant jamais une source par ailleurs pertinente et fiable simplement pour \"faire varier\" artificiellement les types si aucune alternative complémentaire ne passe les trois critères ci-dessus.`,
    "",
    "PRIORITÉ : parmi les encyclopédies de référence, dans cet ordre — Wikipédia (wikipedia.org, idéalement en français, sinon dans une autre langue), puis Encyclopædia Britannica (britannica.com), puis Encyclopædia Universalis (universalis.fr) — choisis en premier/en priorité celle de rang le plus élevé disponible qui correspond clairement au sujet ET remplit les trois critères ci-dessus ; ces trois encyclopédies restent préférables aux autres sources de qualité équivalente pour ce type de fiche. Cela ne dispense JAMAIS de vérifier qu'elles remplissent bien les trois critères (une page hors-sujet ou centrée sur un événement précis reste écartée comme n'importe quelle autre source).",
    "",
...
function buildSourceGuessPrompt(subject) {
  return [
    ...
    "- Priorité aux encyclopédies de référence, dans cet ordre : Wikipédia en français (URL de la forme https://fr.wikipedia.org/wiki/Nom_De_La_Page, avec underscores, sans espace), puis Encyclopædia Britannica (britannica.com), puis Encyclopædia Universalis (universalis.fr) — propose celles qui ont un article correspondant clairement au sujet ;",
    "- Ajoute, quand tu en connais de fiables, 1 à 3 pages institutionnelles/officielles/académiques directement compétentes sur ce sujet précis (site gouvernemental, organisme public) — jamais un blog, un site commercial ou un forum ;",
