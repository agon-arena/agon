...
//
// Canonicalisation d'ordinaux (correctif duplication du 01/09/2026, cf.
// rapport de diagnostic — cas réel "Seconde Guerre mondiale" recréée sous
// "2nde Guerre mondiale"/"Deuxième Guerre mondiale") : appliquée aux DEUX
// côtés de la comparaison, avant l'égalité et avant l'inclusion de mots.
// Volontairement minuscule (1er/2e uniquement — les seules variantes
// rencontrées en pratique) et purement lexicale : jamais un moteur de
// synonymes. isSafeTopicEquivalent (lib/topic-dedup.js) a été testé sur ces
// mêmes variantes avant d'écrire ceci (cf. rapport) : elle reconnaît "La
// Seconde Guerre mondiale" (article filtré comme stopword) mais PAS "2nde"/
// "Deuxième" (un seul mot différent sur 3 fait tomber son score Jaccard à
// 0.5, sous son seuil de 0.75) — elle n'est donc pas branchée ici, une
// canonicalisation dédiée est nécessaire pour ce cas précis.
const ORDINAL_TOKEN_CANONICAL = {
  "1er": "1", "1ere": "1", "1re": "1", "premier": "1", "premiere": "1",
  "2e": "2", "2eme": "2", "2nd": "2", "2nde": "2", "deuxieme": "2", "seconde": "2"
};

function canonicalizeOrdinalTokens(normalized) {
  return String(normalized || "")
    .split(" ")
    .filter(Boolean)
    .map((token) => ORDINAL_TOKEN_CANONICAL[token] || token)
    .join(" ");
}

function matchExistingStarByLabel(proposedLabel, existingStars) {
  const proposed = canonicalizeOrdinalTokens(normalizeTaxonomyName(proposedLabel));
  if (!proposed) return null;
  const proposedWords = new Set(proposed.split(" ").filter(Boolean));
  for (const star of existingStars || []) {
    const candidate = canonicalizeOrdinalTokens(normalizeTaxonomyName(star.name));
