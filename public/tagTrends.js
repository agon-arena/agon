...
function countDistinctSourcesInWindow(item, startDate, endDate) {
...
function isUsableCloudLabel(value) {
  ...
  if (!key || key.length < 3 || GENERIC_CLOUD_LABELS.has(key)) return false;
  return true;
}
...
function getSpecificCloudLabel(item) {
  ...
  const tags = extractRawTagsFromItem(item);
  const fallbackTag = tags.find(isUsableCloudLabel);
...
export function buildSubjectCloudMergeCandidates(items, options = {}) {
  ...
  return (Array.isArray(items) ? items : [])
    .map((item) => {
      const title = String(item?.question || item?.title || "").replace(/\s+/g, " ").trim();
      const subjectId = String(item?.id || "").trim();
      if (!title || !subjectId) return null;
      const currentCount = countDistinctSourcesInWindow(item, currentStart, now);
...
export function buildSubjectTrends(items, options = {}) {
  ...
  const subjects = (Array.isArray(items) ? items : [])
    .map((item) => {
      ...
      getSubjectSourceEntries(item).forEach((entry) => {
        if (entry.date >= currentStart && entry.date < now) currentSources.add(entry.sourceKey);
        if (entry.date >= previousStart && entry.date < currentStart) previousSources.add(entry.sourceKey);
      });
      return {
        tag: cloudLabel,
    ...
    // Un sujet sans tag spécifique exploitable ne doit jamais compléter artificiellement le nuage.
  ...
  const rankedSubjects = mergedSubjects
    .map((subject) => ({
      ...subject,
      trend: getTrendPercent(subject.count, subject.previousCount)
    }))
    .sort((a, b) => (b.count - a.count) || (b.trend - a.trend) || a.tag.localeCompare(b.tag))
    .slice(0, limit);
