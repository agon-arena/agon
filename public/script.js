...
(function initAgonStartupLoader() {
  ...
  runIntroSequence().catch(function() {
    introSequenceDone = true;
    tryHide();
  });
})();
...
function getDebateViewMode() {
  const savedMode = lsGet("debate_view_mode");
...
function setDebateViewMode(mode) {
  ...
  lsSet("debate_view_mode", normalizedMode);
  currentDebateViewMode = normalizedMode;

  if (normalizedMode === "columns") {
    lsSet("debate_column_focus", "split");
...
function getDebateColumnFocus() {
  const saved = lsGet("debate_column_focus");
...
if (pendingColumnFocusScrollMode === "dblclick") {
  ...
  lsSet("debate_column_focus", normalizedMode);
  applyDebateColumnFocusUI();
...
function getEmptyArgumentPasteMeta() {
  return {
    pastedChars: 0,
    pasteRatio: 0,
    manualWritingBadge: false,
...
function computeArgumentPasteMeta(formKey, title, body) {
  if (isAdmin()) {
    return getEmptyArgumentPasteMeta();
  }
...
function renderManualWritingBadge(arg) {
  ...
  return `
    <div class="argument-manual-writing-badge argument-copy-paste-badge" title="Part de texte collé détectée : ${ratio}%">
      <span class="argument-manual-writing-badge-text">${ratio}% copié-collé détecté</span>
    </div>
  `;
}
...
function syncIndexUrlWithOpenIframeModal(modalUrl = "") {
  ...
  try {
    if (normalizedModalUrl) {
      ...
      // pour que le refresh ne ré-ouvre pas les notifs à la place de l'index.
...
function buildIndexEmbedLoadingContent(preview, type) {
  const img = normalizeSourcePreviewData(preview, '').image;
  if (img) {
    return `<img src="${escapeHtml(img)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block;border-radius:inherit;" loading="lazy">`;
  }
  const titles = { x: 'Chargement du post X…', instagram: 'Chargement du post Instagram…' };
  const subs = { x: 'Le post arrive dès que X termine de répondre.', instagram: 'Le post se prépare avant affichage dans la carte.' };
  return buildIndexSocialLoadingPlaceholderHtml(type, titles[type] || 'Chargement…', subs[type] || '');
}
...
function initIndexMediaSwipeEnhancements(root) {
  ...
  if (!initIndexMediaSwipeEnhancements._bound) {
    ...
    const renderShellIndex = async (shell, nextIndex) => {
      ...
      const isSource = String(nextItem?.type || '').trim() === 'source'
      ...
      if (isSource && !cachedPreview) {
        const indexAtRender = normalizedIndex;
      ...
      const nextItemPreview = cachedPreview;
    ...
    document.addEventListener("touchend", async (event) => {
      ...
      const activeVideo = shell.querySelector('video');
...
async function renderIndexXShell(shell) {
  ...
  try {
    ...
    // Cap la hauteur dans le carousel
    if (shell.closest('.theme-horizontal-inner')) {
      const isMulti = !!shell.closest('.index-media-swipe-shell');
      if (window.innerWidth >= 769 || isMulti) {
        shell.style.maxHeight = '169px';
        shell.style.minHeight = '0';
        shell.style.overflow = 'hidden';
        const wrapper = shell.closest('.debate-card-media-x');
        if (wrapper) { wrapper.style.maxHeight = '169px'; wrapper.style.overflow = 'hidden'; }
      }
    }
  } catch (error) {
    shell.dataset.rendered = 'failed';
...
function revealIndexInstagramEmbed(shell, embed, loading) {
  ...
  if (loading) loading.style.display = 'none';
  const igFallback = shell.querySelector('[data-index-instagram-fallback]');
  if (igFallback) igFallback.style.display = 'none';
  ...
  if (shell.closest('.theme-horizontal-inner')) {
    ...
    if (window.innerWidth >= 769 || isMulti) {
      ...
      shell.style.minHeight = '0';
      shell.style.overflow = 'hidden';
      const wrapper = shell.closest('.debate-card-media-instagram');
      if (wrapper) { wrapper.style.maxHeight = '169px'; wrapper.style.overflow = 'hidden'; }
    }
  }
}
...
function updateIndexRowContextOpenState(row) {
  if (!row) return;
...
function reserveIndexRowHeightForContextOpen(article, metaEl) {
  ...
  if (!article.classList.contains('index-card-context-open')) return;

  const rowRect = row.getBoundingClientRect();
  const articleRect = article.getBoundingClientRect();
  const rowStyle = getComputedStyle(row);
  const bottomPadding = parseFloat(rowStyle.paddingBottom) || 0;
  const articleTop = Math.max(0, articleRect.top - rowRect.top);
  let targetArticleHeight = articleRect.height || article.offsetHeight || 0;

  if (metaEl) {
    const metaStyle = getComputedStyle(metaEl);
    const maxHeightValue = parseFloat(metaStyle.maxHeight);
    const maxVisibleHeight = Number.isFinite(maxHeightValue) && maxHeightValue > 0 ? maxHeightValue : Infinity;
    const targetMetaHeight = Math.min(metaEl.scrollHeight || 0, maxVisibleHeight);
    const currentMetaHeight = metaEl.getBoundingClientRect().height || 0;
    if (targetMetaHeight > currentMetaHeight) {
      targetArticleHeight += targetMetaHeight - currentMetaHeight;
    }
  }
...
function _showSecondaryTagsInCloud(primaryTag, primaryDebateId) {
  ...
  // Pré-construit un index normalizedTag → [débats] pour éviter O(n²)
  const tagIndex = new Map();
  for (const d of debatesCache) {
    if (String(d.id) === String(primaryDebateId)) continue;
    for (const rawTag of extractRawTagsFromItem(d)) {
      const norm = normalizeTag(rawTag);
      if (!tagIndex.has(norm)) tagIndex.set(norm, []);
      tagIndex.get(norm).push(d);
    }
  }
  ...
  for (const rawTag of extractRawTagsFromItem(primaryDebate)) {
    ...
    const others = tagIndex.get(norm) || [];
...
function renderDebatesList(debates) {
  ...
  if (hasRows) {
    ...
    document.querySelectorAll(
      '.theme-horizontal-inner [data-index-x-shell], .theme-horizontal-inner [data-index-instagram-shell]'
    ).forEach(shell => {
      const isMulti = !!shell.closest('.index-media-swipe-shell');
      if (window.innerWidth >= 769 || isMulti) {
        shell.style.maxHeight = '169px';
        shell.style.minHeight = '169px';
        shell.style.overflow = 'hidden';
      }
    });
