...
test("findExistingQuizMaster filtre les candidats par isMasterEligibleQuiz (jamais un simple exact-match aveugle)", () => {
  ...
  // Lecture en 2 temps depuis l'audit egress du 01/09/2026, V2 le 08/09/2026
  // (cf. test/daily-quiz-egress-v1.test.js) : `questions` n'est plus dans le
  // select initial, l'éligibilité est vérifiée sur le résumé léger
  // (summary:daily_quiz_question_summaries) — même garde-fou qu'avant,
  // jamais un simple exact-match aveugle. Contexte progressif
  // (progressiveStatus/curriculum) transmis depuis le 07/09/2026 (canari de
  // pré-génération Batch), cf. test/daily-quiz-egress-v1.test.js.
  assert.match(fnBody, /if \(isMasterEligibleQuiz\(summaryQuestions, \{ progressiveStatus: row\.progressive_status, curriculum: row\.curriculum \}\)\)/, "un candidat trouvé ne doit être retenu comme master que s'il porte pedagogicalRank ET respecte le seuil de son propre statut progressif");
});
